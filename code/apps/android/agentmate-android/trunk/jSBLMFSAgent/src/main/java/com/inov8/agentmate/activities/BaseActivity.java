...
public class BaseActivity extends Activity {
    ...
    public void askOtp(final Bundle bundle, final Class<?> nextClass,
                       final boolean closeAcitivty, String productId) {
        DisplayMetrics displaymetrics = new DisplayMetrics();
        getWindowManager().getDefaultDisplay().getMetrics(displaymetrics);
        int height = displaymetrics.heightPixels;
        int setterHeight = 0;
        ...
        if(productId.equals("10245156") || productId.equals("10245157"))
        userInput.setFilters(new InputFilter[]{new InputFilter.LengthFilter(Constants.MAX_LENGTH_PIN)});
else
        userInput.setFilters(new InputFilter[]{new InputFilter.LengthFilter(Constants.MAX_LENGTH_THIRD_PARTY_OTP)});
        ...
        btnOk.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View arg0) {
                ...
                if (!testValidity(userInput)) {
                    return;
                }  else if(productId.equals("10245156") || productId.equals("10245157")){
                    if (userInput.getText().toString().length() < Constants.MAX_LENGTH_BOP_OTP) {
                        userInput.setError(Constants.Messages.INVALID_BOP_OTP);
                        return;
                    }
