...
public class CashOutConfirmationActivity extends BaseCommunicationActivity implements MpinInterface {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            if (cashOutType.equals(Constants.CASH_OUT_BY_TRX_ID) || cashOutType.equals(Constants.CASH_OUT_BY_IVR) || product.getId().equals(Constants.PRODUCT_ID_CASH_WITH_DRAWAL)) {
...
//            decideBvs();
            ...
            if (Byte.parseByte(product.getFlowId()) == Constants.FLOW_ID_CASH_OUT_BY_TRX_ID) {
                currCommand = Constants.CMD_CASH_OUT_BY_TRX_ID;
            } else if (Byte.parseByte(product.getFlowId()) == Constants.FLOW_ID_CASH_OUT_BY_IVR) {
...
//        if (cashOutType.equals(Constants.CASH_OUT_BY_IVR))
        ...
        if (currCommand == Constants.CMD_HRA_CASH_WITHDRAWAL || currCommand == Constants.CMD_CASH_OUT) {
            new HttpAsyncTask(CashOutConfirmationActivity.this).execute(
                    ...
                    cashOutType.equals(Constants.CASH_OUT_BY_TRX_ID) ? transactionInfo.getTrxid() : "", otpCode != null ? AesEncryptor.encrypt(otpCode) : "", cashOutType.equals(Constants.CASH_OUT_BY_IVR) ? "1" : "");
