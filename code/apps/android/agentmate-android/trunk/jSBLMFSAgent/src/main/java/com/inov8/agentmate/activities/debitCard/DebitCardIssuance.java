...
import com.inov8.agentmate.activities.proofOfLife.ProofOfLifeInputActivity;
...
public class DebitCardIssuance extends BaseCommunicationActivity implements View.OnClickListener {
    ...
    private TextView lblHeading, tv_mobileNo, tv_cnic, tv_card_embossing_name, tv_mailing_address,lbCardTypes;
    ...
    ArrayAdapter<String> cardTypesAdapter;
    private Spinner inputSpinnerCardTypes;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        inputSpinnerCardTypes = findViewById(R.id.spDebitCardIssue);
        lbCardTypes = findViewById(R.id.lbl_debit_card_type);



        if (!ApplicationData.isLoginFlow) {
//            selectedCardType = "201";
            reSetUI();
...
//        else {
//            ArrayList<String> cardTypesList = new ArrayList<>();
//            for (int i = 0; i < ApplicationData.listCardTypes.size(); i++) {
//                cardTypesList.add(ApplicationData.listCardTypes.get(i).getName());
//            }
//            cardTypesAdapter = new ArrayAdapter<String>(this, android.R.layout.simple_spinner_item, cardTypesList);
//            inputSpinnerCardTypes.setAdapter(cardTypesAdapter);
//        }
        setDebitCardIssueSpinner();
    ...
    private void reSetUI() {
        ...
        inputSpinnerCardTypes.setVisibility(View.VISIBLE);
        lbCardTypes.setVisibility(View.VISIBLE);
...
//    }
    ...
    public void processRequest() {
        ...
        try {
            ...
            if (currentCommand == Constants.CMD_DEBIT_CARD_FETCH_SEGMENTS) {
                new HttpAsyncTask(DebitCardIssuance.this).execute(Constants.CMD_DEBIT_CARD_FETCH_SEGMENTS + "");
...
//                inputSpinnerCardTypes.setEnabled(false);
//                inputSpinnerCardTypes.setClickable(false);
                ...
                if(segmentId.equals("") ) {
                    new HttpAsyncTask(this).execute(
                            Constants.CMD_DEBIT_CARD_CONFIRMATION + "", encMPIN, mobileNumber, cnic, customerembossingName, customerMailingAddress, selectedCardType, ApplicationData.userId, ApplicationData.selectedOpenAccountSegmentId);
                }else {
                    new HttpAsyncTask(this).execute(
                            Constants.CMD_DEBIT_CARD_CONFIRMATION + "", encMPIN, mobileNumber, cnic, customerembossingName, customerMailingAddress, selectedCardType, ApplicationData.userId, segmentId);
                }
            }
        } catch (Exception e) {
            hideLoading();
    ...
    public void processResponse(HttpResponseModel response) {
        try {
            ...
            if (table != null && table.containsKey(Constants.KEY_LIST_ERRORS)) {
                ...
                if (code.equals(Constants.ErrorCodes.DEBIT_CARD_PENDING_REQUEST_ERROR)) {
                ...
                } else if (code.equals(Constants.ErrorCodes.OTP_INVALID)) {
                    PopupDialogs.createAlertDialog(message.getDescr(),
                            AppMessages.ALERT_HEADING, PopupDialogs.Status.ERROR,
                            DebitCardIssuance.this, new PopupDialogs.OnCustomClickListener() {
                                @Override
                ...
                if (currentCommand== Constants.CMD_DEBIT_CARD_FETCH_SEGMENTS) {
                    ApplicationData.listCardTypes = null;
                    ApplicationData.listCardTypes = (ArrayList<SegmentModel>) table.get(Constants.TAG_CARDTYPES);
                    ArrayList<String> cardTypesList = new ArrayList<>();
                    for (int i = 0; i < ApplicationData.listCardTypes.size(); i++) {
                        cardTypesList.add(ApplicationData.listCardTypes.get(i).getName());
                    }
...
//                            verifyOtp();
                        ...
                        try {
                            segmentId = table.get(Constants.ATTR_SEGMENT_ID).toString();
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent intent) {
        ...
        if (requestCode == Constants.CMD_DEBIT_CARD) {
            if (ApplicationData.isTermsAndConditionAccepted) {
                ...
                chkFee.setVisibility(View.GONE);
            }
...
//            startActivity(i);
    ...
    private void setDebitCardIssueSpinner() {
//        Spinner spDebitCardIssue = (Spinner) findViewById(R.id.spDebitCardIssue);
...
//        spDebitCardIssue.setAdapter(dataAdapter);


        inputSpinnerCardTypes.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {

            @Override
            public void onItemSelected(AdapterView<?> arg0, View arg1,
                                       int position, long arg3) {
                selectedCardType = ApplicationData.listCardTypes.get(arg0.getSelectedItemPosition()).getCode();
