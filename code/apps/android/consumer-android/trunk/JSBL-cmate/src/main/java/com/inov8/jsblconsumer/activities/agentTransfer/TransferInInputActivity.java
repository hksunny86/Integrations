...
public class TransferInInputActivity extends BaseCommunicationActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            titleImplementation(null, product.getName(), null, TransferInInputActivity.this);
    ...
    public void processRequest() {
        if (!haveInternet()) {
            PopupDialogs.createAlertDialog(AppMessages.INTERNET_CONNECTION_PROBLEM,
                    AppMessages.ALERT_HEADING, TransferInInputActivity.this, PopupDialogs.Status.ERROR);
            return;
        }

        showLoading("Please Wait", "Processing...");
        new HttpAsyncTask(TransferInInputActivity.this).execute(
                Constants.CMD_TRANSFER_IN_INFO + "", product.getId(), input1.getText().toString());
    }
        ...
        try {
            ...
            if (table != null && table.containsKey(Constants.KEY_LIST_ERRORS)) {
                ...
                PopupDialogs.createAlertDialog(message.getDescr(), getString(R.string.alertNotification),
                        TransferInInputActivity.this, PopupDialogs.Status.ERROR);

            } else {
                TransactionInfoModel transactionInfo = TransactionInfoModel.getInstance();
                transactionInfo.TransferIn(
                        table.get(XmlConstants.Attributes.BBACID).toString(),
                ...
                Intent intent = new Intent(getApplicationContext(), TransferInConfirmationActivity.class);
