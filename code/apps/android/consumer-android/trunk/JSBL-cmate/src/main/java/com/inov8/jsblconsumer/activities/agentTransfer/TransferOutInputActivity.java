...
public class TransferOutInputActivity extends BaseCommunicationActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            titleImplementation(null, product.getName(), null, TransferOutInputActivity.this);
    ...
    public void processRequest() {
        if (!haveInternet()) {
            PopupDialogs.createAlertDialog(AppMessages.INTERNET_CONNECTION_PROBLEM,
                    AppMessages.ALERT_HEADING, TransferOutInputActivity.this, PopupDialogs.Status.ERROR);
            return;
        }

        showLoading("Please Wait", "Processing...");
        new HttpAsyncTask(TransferOutInputActivity.this).execute(
                Constants.CMD_TRANSFER_OUT_INFO + "", product.getId(),
        ...
        try {
            ...
            if (table != null && table.containsKey(Constants.KEY_LIST_ERRORS)) {
                ...
                PopupDialogs.createAlertDialog(message.getDescr(), getString(R.string.alertNotification),
                        TransferOutInputActivity.this, PopupDialogs.Status.ERROR);

            } else {
                TransactionInfoModel transactionInfo = TransactionInfoModel.getInstance();
                transactionInfo.TransferOut(table.get(
                        XmlConstants.Attributes.BBACID).toString(), table.get(
                        XmlConstants.Attributes.COREACID).toString(), table.get(
                        XmlConstants.Attributes.COREACTL).toString(), table.get(
                        XmlConstants.Attributes.TXAM).toString(), table.get(
                        XmlConstants.Attributes.TXAMF).toString(), table.get(
                        XmlConstants.Attributes.TPAM).toString(), table.get(
                        XmlConstants.Attributes.TPAMF).toString(), table.get(
                        XmlConstants.Attributes.CAMT).toString(), table.get(
                        XmlConstants.Attributes.CAMTF).toString(), table.get(
                        XmlConstants.Attributes.TAMT).toString(), table.get(
                        XmlConstants.Attributes.TAMTF).toString());

                Intent intent = new Intent(getApplicationContext(),
                        TransferOutConfirmationActivity.class);
