...
import com.mx.finger.alg.MxFingerAlgHYF2;
...
public class LAPI {
    ...
    public static final String keyPath = "/sdcard/.bak/licensekey.txt";
    private final MxFingerAlgHYF2 mxFingerAlg;
    private int mxFingerAlgHandler;

    //------------------------------------------------------------------------------------------------//
    // Purpose   : This function initializes the Fingerprint Recognition SDK Library.
    // Function  : Initialize
    // Arguments : void
    //		(In) : String uuid :
    ...
    //------------------------------------------------------------------------------------------------//
    public int Initialize(Context mContext, String keyPath) {
        return mxFingerAlg.Initialize(mContext, keyPath);
    }

    //------------------------------------------------------------------------------------------------//
    // Purpose   : This function finalizes the Fingerprint Recognition SDK Library.
    // Function  : Finalize
    // Arguments :
    //      (In) : int handle : handle returned from function "Initialize()"
    // Return    : int
    //			      If successful, return 1, else 0
    //------------------------------------------------------------------------------------------------//
    public int Finalize(int handle) {
        return mxFingerAlg.Finalize(handle);
    }
    ...
    public static final int TABLET_DEFAULT = 3;
    public static final int TABLET_BH701 = 1;
    public static final int TABLET_BH502 = 2;
    public static final int TABLET_BH702 = 3;
    ...
    public LAPI(Activity a) {
        m_content = a;
        mxFingerAlg = new MxFingerAlgHYF2(WIDTH, HEIGHT);
    }

    private void enableFinger(boolean on) {
    ...
    protected void POWER_ON(int tablet) {
        FileReader inCmd;
        try {
            if (tablet == TABLET_BH701) {
                inCmd = new FileReader(PWFILE_BH701);
                inCmd.read();
                inCmd.close();
            } else if (tablet == TABLET_BH502) {
                inCmd = new FileReader(PWFILE_BH502);
                inCmd.read();
                inCmd.close();
            } else if (tablet == TABLET_BH702) {
                enableFinger(true);
            } else return;
    ...
    protected void POWER_OFF(int tablet) {
        ...
        try {
            if (tablet == TABLET_BH701) {
                closefr = new FileWriter(PWFILE_BH701);
                closefr.write("1");
                closefr.close();
            } else if (tablet == TABLET_BH502) {
                closefr = new FileWriter(PWFILE_BH502);
                closefr.write("1");
                closefr.close();
            } else if (tablet == TABLET_BH702) {
                enableFinger(false);
    ...
    public int OpenDeviceEx() {
        mxFingerAlgHandler = Initialize(m_content.getApplication(), keyPath);
        if (mxFingerAlgHandler < 0) {
            Log.e(TAG, "Initialize Fingerprint Recognition SDK Library Error !! ，Code = " + mxFingerAlgHandler);
            return 0;
        }
        int ret = 0;
        POWER_ON(tabletNo);
    ...
    // Return    : int
    //			      If successful, return the count of minutiae point,
    ...
    //------------------------------------------------------------------------------------------------//
    // Function  : GetImageMinutiae
    // Arguments :
    //      (In) : int device : handle returned from function "OpenDevice()"
    //  (In/Out) : byte[] imageBit8 : image captured from this device，size is 256*360,
    //              used to extract features, etc.
    //  (In/Out) : byte[] imageBit24 : image with minutiae, size is 256*360*3 ,
    //              For display only.
    // Return    : int
    //			      If successful, return the count of image minutiae,
    //				  if not calibrated(TCS1/2), return -2,
    //						else, return  0 or negative
    //------------------------------------------------------------------------------------------------//
    public int GetImageMinutiae(int device, byte[] imageBit8, byte[] imageBit24) {
        int i = GetImage(device, imageBit8);
        if (i != TRUE) {
            return i;
        }
        System.arraycopy(imageBit8, 0, imageBit24, 0, WIDTH * HEIGHT);
        return DrawMinutiaePoint(device, imageBit24);
    }
    ...
    public int CreateANSITemplate(int device, byte[] image, byte[] itemplate) {
        return mxFingerAlg.CreateANSITemplate(device, image, itemplate);
    }
    ...
    public int CreateISOTemplate(int device, byte[] image, byte[] itemplate) {
        return mxFingerAlg.CreateISOTemplate(device, image, itemplate);
    }
    ...
    public int GetImageQuality(int device, byte[] image) {
        return mxFingerAlg.GetImageQuality(device, image);
    }
    ...
    public int GetNFIQuality(int device, byte[] image) {
        return mxFingerAlg.GetNFIQuality(device, image);
    }
    ...
    public int CompareTemplates(int device, byte[] itemplateToMatch, byte[] itemplateToMatched) {
        return mxFingerAlg.CompareTemplates(device, itemplateToMatch, itemplateToMatched);
    }
    ...
    public int SearchingANSITemplates(int device, byte[] itemplateToSearch,
        ...
        return mxFingerAlg.SearchingANSITemplates(device, itemplateToSearch, numberOfDbTemplates, arrayOfDbTemplates, scoreThreshold);
    }
    ...
    public int SearchingISOTemplates(int device, byte[] itemplateToSearch,
                                     int numberOfDbTemplates, byte[] arrayOfDbTemplates, int scoreThreshold) {
        return mxFingerAlg.SearchingISOTemplates(device, itemplateToSearch, numberOfDbTemplates, arrayOfDbTemplates, scoreThreshold);
    }
    ...
    public int DrawMinutiaePoint(int device, byte[] rawImage) {
        return mxFingerAlg.DrawMinutiaePoint(device, rawImage);
    }
}
