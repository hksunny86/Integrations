...
public class TransferOutInputActivity extends BaseCommunicationActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			lblHeading.setText("Transfer Out");
	...
	public void processRequest() {
		...
		new HttpAsyncTask(TransferOutInputActivity.this).execute(
				...
				"pin", ApplicationData.agentMobileNumber, input1.getText().toString(),
		...
		try {
			...
			if (table != null && table.containsKey(Constants.KEY_LIST_ERRORS)) {
				...
				transactionInfo.TransferOut(table.get(
						Constants.ATTR_BBACID).toString(), table.get(
						Constants.ATTR_COREACID).toString(), table.get(
						Constants.ATTR_COREACTL).toString(), table.get(
