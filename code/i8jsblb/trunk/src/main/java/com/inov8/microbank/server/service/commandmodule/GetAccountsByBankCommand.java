...
public class GetAccountsByBankCommand extends BaseCommand
{
	...
	String bankId ;
	String customerRelAccountsInqResponse = null ;

	protected final Log logger = LogFactory.getLog(GetAccountsByBankCommand.class);
	...
	public void execute() throws CommandException
	{
		...
		{
			logger.debug("Start of GetAccountsByBankCommand.execute()");
		}
...
//					LinkedHashMap<String,SortingOrder> sortingOrderMap = new LinkedHashMap();
					...
					sMAcctInfoListViewModel.setBankId(Long.parseLong(bankId));
...
//					searchBaseWrapper.setSortingOrderMap(sortingOrderMap);
					...
					// Following code will fetch the account from the Bank service ... i.e in case of BANK
					
					if( smAcctInfoList != null )
					{
						SmAcctInfoListViewModel smAccountInfoModel = smAcctInfoList.get(0);
						
						if( smAccountInfoModel.getIsBank() )
						{
							...
							baseWrapper.putObject(CommandFieldConstants.KEY_BANK_ID, bankId);
							
							CommandManager commandManager = this.getCommandManager();
							customerRelAccountsInqResponse = commandManager.executeCommand(baseWrapper, CommandFieldConstants.CMD_CUSTOMER_ACCOUNT_RELATIONSHIP_INQUIRY);
	...
	public ValidationErrors validate(ValidationErrors validationErrors) throws CommandException
	{
		...
		if(!validationErrors.hasValidationErrors())
		{
			...
			validationErrors = ValidatorWrapper.doInteger(bankId, validationErrors, "Bank Id");
		}
		if(logger.isDebugEnabled())
		{
	...
	private String toXML()
	{
		...
		if(toXMLFlag)
		{
			...
			while( smAcctInfoListViewModelIte.hasNext() )
			{
				...
				while( accountsIte.hasNext() )
				{
					...
					if( !localSmAcctInfoListViewModel.getIsBank() )
					{
						...
						.append(TAG_SYMBOL_EQUAL)
						.append(TAG_SYMBOL_QUOTE)
						.append(localSmAcctInfoListViewModel.getName())
						.append(TAG_SYMBOL_QUOTE);
			...
			strBuilder.append(customerRelAccountsInqResponse);
