...
public class TransferOutCommand extends BaseCommand  {
	
	protected final Log logger = LogFactory.getLog(TransferOutCommand.class);
	...
	public void execute() throws CommandException {
		...
		if(logger.isDebugEnabled()) {
			logger.debug("Start of TransferOutCommand.execute()");
		}
		...
		try {
			...
			if(!validationErrors.hasValidationErrors()) {
				...
				commonCommandManager.checkProductLimit(null,productModel.getProductId(), appUserModel.getMobileNo(), deviceTypeModel.getDeviceTypeId(), Double.parseDouble(amount), null, null, workFlowWrapper.getHandlerModel());				
...
//					fetchTitle(switchWrapper);				
					
					logger.info("[TransferOutCommand.execute] \nProduct ID: " + productId + " \nLogged In AppUserID:" + appUserModel.getAppUserId() + 
...
//				smartMoneyAccountModel = workFlowWrapper.getSmartMoneyAccountModel();
				...
				commonCommandManager.sendSMS(workFlowWrapper);
					...
					logger.error("[TransferOutCommand.prepare] Throwing Exception for Product ID: " + productId + " Logged In AppUserID:" + appUserModel.getAppUserId() + " Customer Mobile No:" + mobileNo + " Trx Amount: " + amount + " Commission: " + commissionAmount);
				...
				if(logger.isErrorEnabled()) {
					logger.error("[TransferOutCommand.prepare] Throwing Exception for Product ID: " + productId + " Logged In AppUserID:" + appUserModel.getAppUserId() + " Customer Mobile No:" + mobileNo + " Trx Amount: " + amount + " Commission: " + commissionAmount + " Exception details: " + ExceptionProcessorUtility.prepareExceptionStackTrace(ex));
				}
				...
				if(logger.isErrorEnabled()) {
					...
					logger.error("[TransferOutCommand.prepare] Throwing Exception for Product ID: " + productId + " Logged In AppUserID:" + appUserModel.getAppUserId() + " Customer Mobile No:" + mobileNo + " Trx Amount: " + amount + " Commission: " + commissionAmount + " Exception details: " + ExceptionProcessorUtility.prepareExceptionStackTrace(wex));
				}
		...
		if(logger.isDebugEnabled()) {
			logger.debug("End of TransferOutCommand.execute()");
		}
	}
	...
	public void prepare(BaseWrapper baseWrapper) {
		...
		if(logger.isDebugEnabled()) {
			logger.debug("Start of TransferOutCommand.prepare()");
		}
		...
		coreAccountNick = this.getCommandParameter(baseWrapper, CommandFieldConstants.KEY_ACCOUNT_TITLE_AGNETMATE);	
		coreAccountNumber = this.getCommandParameter(baseWrapper, CommandFieldConstants.KEY_ACCOUNT_NUMBER_AGNETMATE);	
		...
		try {
			...
			if(logger.isErrorEnabled()) {
				...
				logger.error("TransferOutCommand.prepare] Exception occured for ProductID: " + productId + " AppUserID: " + appUserModel.getAppUserId() + " Exception Details: " + ExceptionProcessorUtility.prepareExceptionStackTrace(ex));
			}
...
//		commissionAmount = this.getCommandParameter(baseWrapper, CommandFieldConstants.KEY_COMM_AMOUNT);
		...
		logger.info("[TransferOutCommand.prepare] Product ID: " + productId + " Logged In AppUserID:" + appUserModel.getAppUserId() + " Customer Mobile No:" + mobileNo + " Walkin Customer Mobile No:" + walkInCustomerMobileNumber + " Trx Amount: " + amount + " Commission: " + commissionAmount);
		...
		if(logger.isDebugEnabled()) {
			logger.debug("End of TransferOutCommand.prepare()");
		}
	}
	...
	public ValidationErrors validate(ValidationErrors validationErrors) throws CommandException {
		...
		if(logger.isDebugEnabled()) {
			logger.debug("Start of TransferOutCommand.validate()");
		}
		...
		if(logger.isDebugEnabled()) {
			logger.debug("End of TransferOutCommand.validate()");
		}
	...
	private String toMobileXMLResponse() {
		...
		String totalAmount = Formatter.formatDouble(workFlowWrapper.getCommissionAmountsHolder().getTotalAmount());
		String coreAccountNumber = workFlowWrapper.getSwitchWrapper().getAccountInfoModel().getAccountNo();
		...
		responseBuilder.append(
				"<trn "+
					...
					" COREACID='"+StringEscapeUtils.escapeXml(this.coreAccountNumber)+"'"+
					" COREACTL='"+StringEscapeUtils.escapeXml(this.coreAccountNick)+"'"+
				    ...
				    " TXAM='"+amount+"'"+
				    " TXAMF='"+Formatter.formatDoubleByPattern(Double.valueOf(amount), "#,###.00")+"'"+
					" DATE='"+date+"'"+
					" DATEF='"+sdf.format(_transactionModel.getCreatedOn())+"'"+
					" TIMEF='"+timef.format(_transactionModel.getCreatedOn())+"'"+
					" BALF='"+Formatter.formatDoubleByPattern((Double)workFlowWrapper.getObject("OLA_BALANCE"), "#,###.00")+"'>"+
				"</trn>");
...
//		logger.debug("Found Smart Money Account "+ smartMoneyAccountModel.getName());
	 ...
	 */
	private Boolean fetchTitle(SwitchWrapper switchWrapper) throws CommandException {
		
		Boolean titleFetched = Boolean.FALSE;
		
   	  	try {
   	  		
   	  		titleFetched = getCommonCommandManager().titleFetch(switchWrapper);
			
   	  		if(!titleFetched) {
   	  			throw new CommandException("Title Fetch Failed", ErrorCodes.INVALID_USER_ACCOUNT, null);
   	  		} 
