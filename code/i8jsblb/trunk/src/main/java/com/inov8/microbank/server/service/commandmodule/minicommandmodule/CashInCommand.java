...
public class CashInCommand extends MiniBaseCommand {
	...
	public void prepare(BaseWrapper baseWrapper) {
		...
		logger.info("P2PCommand.prepare Started...........") ;
...
//			AppUserPeerModel appUserPeerModel = new AppUserPeerModel() ;
			...
			{
				logger.error("Exception thrown by CashInCommand.");
			...
			{
				logger.error("Exception thrown by TopupCommand.execute() Line:174 --- Not a Variable Product ");
			...
			if( searchBaseWrapper.getCustomList().getResultsetList() != null && searchBaseWrapper.getCustomList().getResultsetList().size() > 0 )
			{
				...
				//smartMoneyAccountModel.getStatusId() != null &&
				...
				else if(smartMoneyAccountModel.getStatusId().equals(OlaStatusConstants.ACCOUNT_STATUS_BLOCKED))
					throw new CommandException("Your Account is Blocked.", ErrorCodes.COMMAND_EXECUTION_ERROR,ErrorLevel.MEDIUM);
				else if(smartMoneyAccountModel.getStatusId().equals(OlaStatusConstants.ACCOUNT_STATUS_IN_ACTIVE))
					throw new CommandException("Your Account is not Active.", ErrorCodes.COMMAND_EXECUTION_ERROR,ErrorLevel.MEDIUM);
				else if(smartMoneyAccountModel.getStatusId().equals(OlaStatusConstants.ACCOUNT_STATUS_DORMANT))
					throw new CommandException("Your Account is in Dormant State.", ErrorCodes.COMMAND_EXECUTION_ERROR,ErrorLevel.MEDIUM);
...
*/
					...
					miniTransactionModel.setCommandId( Long.valueOf(CommandFieldConstants.CMD_MINI_CI)) ;
					miniTransactionModel.setAppUserId( ThreadLocalAppUser.getAppUserModel().getAppUserId() ) ;
					...
					response = this.getMessageSource().getMessage("MINI.CIPINSMS", new Object[]{receiverName,firstParam, miniTransactionModel.getBAMT(), miniTransactionModel.getCAMT(), miniTransactionModel.getTAMT(),PIN,
...
//		}
	...
	{
		if(null == ThreadLocalAppUser.getAppUserModel().getRetailerContactId())
		{
			response = this.getMessageSource().getMessage("MINI.transactionNotAllowed", null, null) ;
			throw new CommandException(response, ErrorCodes.COMMAND_EXECUTION_ERROR,ErrorLevel.MEDIUM);
		}
		...
		{
			response = this.getMessageSource().getMessage("MINI.InvalidP2PFormat", new Object[]{}, null) ;
			throw new CommandException(response, ErrorCodes.COMMAND_EXECUTION_ERROR,ErrorLevel.MEDIUM);
		}

		if(  (txAmount == null || txAmount.equalsIgnoreCase("") ))
		{
			response = this.getMessageSource().getMessage("MINI.InvalidP2PFormat", null, null) ;
			throw new CommandException(response, ErrorCodes.COMMAND_EXECUTION_ERROR,ErrorLevel.MEDIUM);
		}
		...
		{
			e.printStackTrace();
			response = this.getMessageSource().getMessage("MINI.InvalidCIFormat", null, null) ;
			throw new CommandException(response, ErrorCodes.COMMAND_EXECUTION_ERROR,ErrorLevel.MEDIUM);
		}
		...
		{
			logger.info("$$$$$$ TopupCommand.doValidate() Line:127 Product Id in MiniCommandKeyword table is not mentioned.");
			response = this.getMessageSource().getMessage("MINI.GeneralError", new Object[]{firstParam}, null) ;
			throw new CommandException(response, ErrorCodes.COMMAND_EXECUTION_ERROR,ErrorLevel.MEDIUM);
		}
