...
public class P2PCommand extends MiniBaseCommand
{
			 ...
			 */
				...
				if( searchBaseWrapper.getCustomList().getResultsetList() != null && searchBaseWrapper.getCustomList().getResultsetList().size() > 0 ) 
				{
					...
					else // Send the temporary Verifly Pin 
					{
						...
						miniTransactionModel.setCommandId( Long.valueOf(CommandFieldConstants.CMD_MINI_P2P)) ;
						miniTransactionModel.setAppUserId( ThreadLocalAppUser.getAppUserModel().getAppUserId() ) ;
						...
						try
						{
							...
							miniTransactionModel.setBAMT( Double.parseDouble( MiniXMLUtil.getTagTextValue(response, MiniXMLUtil.BILL_AMOUNT_UNFORMATED_NODEREF) )) ;
							miniTransactionModel.setTAMT( Double.parseDouble( MiniXMLUtil.getTagTextValue(response, MiniXMLUtil.TAMT_NODEREF) )) ;
						...
						response = this.getMessageSource().getMessage("MINI.P2PPINSMS", new Object[]{receiverName,firstParam, miniTransactionModel.getBAMT(), miniTransactionModel.getCAMT(), miniTransactionModel.getTAMT(),PIN,
