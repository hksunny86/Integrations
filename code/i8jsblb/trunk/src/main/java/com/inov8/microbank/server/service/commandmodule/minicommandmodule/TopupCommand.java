...
import com.inov8.microbank.common.util.ZongShortCodeConstants;
...
public class TopupCommand extends MiniBaseCommand
{
	protected String mobileNo;
	protected HashMap parameters = null ;
	protected String mobileNoParam ;
	...
	public void prepare(BaseWrapper baseWrapper)
	{
		...
		if( !doPayBill )
		{
			mobileNoParam = (String)parameters.get( CommandFieldConstants.KEY_PARAM + CommandFieldConstants.PARAM_COUNTER_FIRST_VAL ) ;
...
//			firstParam = this.getCommandParameter(baseWrapper, CommandFieldConstants.KEY_PARAM_SMS_TEXT);
	...
	public void doValidate() throws CommandException
	{
		...
		if(  (mobileNoParam == null || mobileNoParam.equalsIgnoreCase("") ))
		{
		...
		{
			response = this.getMessageSource().getMessage("MINI.InvalidPIN", new Object[]{mobileNoParam}, null) ;
			throw new CommandException(response, ErrorCodes.COMMAND_EXECUTION_ERROR,ErrorLevel.MEDIUM);					
		}
...
//		}		
	...
	public void execute() throws CommandException
	{
		...
		try
		{
			...
			{
				// Load Product model to check whether the product is active or not
			if( mobileNoParam.startsWith( ZongShortCodeConstants.ZONG ) )
					productModel.setProductId( Long.parseLong(this.getMessageSource().getMessage(ZongShortCodeConstants.ZONG_PRODUCT_ID_KEY, null, null)) ) ;
...
//					}
					...
					if( doPayBill ) // Call the BillPaymentCommand
					{
						...
						getCommonCommandManager().checkProductLimit(null, productModel.getProductId(), ThreadLocalAppUser.getAppUserModel().getMobileNo(), Long.valueOf(deviceTypeId), Double.parseDouble(txAmount), null, null, handlerModel);
						...
						baseWrapper.putObject( CommandFieldConstants.KEY_MOB_NO, mobileNoParam ) ;
						baseWrapper.putObject( CommandFieldConstants.KEY_CUST_CODE, mobileNoParam ) ;
						baseWrapper.putObject( CommandFieldConstants.KEY_PIN, pin ) ;
						
						baseWrapper.putObject( CommandFieldConstants.KEY_TX_AMOUNT, miniTransactionModel.getBAMT() ) ;
						...
						response = this.getCommandManager().executeCommand(baseWrapper, CommandFieldConstants.CMD_CVPS_TRANS);
...
//						baseWrapper.putObject( CommandFieldConstants.KEY_BILL_AMOUNT, txAmount ) ;
						...
						saveMiniCommandLog( Long.parseLong(CommandFieldConstants.CMD_TOPUP_TX_INFO));
						response = this.getCommandManager().executeCommand(baseWrapper, CommandFieldConstants.CMD_TOPUP_TX_INFO);
						...
						miniTransactionModel.setCommandId( Long.valueOf(CommandFieldConstants.CMD_MINI_TOPUP)) ;
						miniTransactionModel.setAppUserId( ThreadLocalAppUser.getAppUserModel().getAppUserId() ) ;
						miniTransactionModel.setTimeDate(new Date()) ;
						miniTransactionModel.setMobileNo( this.mobileNo );
						miniTransactionModel.setSmsText( mobileNoParam + " " + txAmount ) ;
						miniTransactionModel.setMiniTransactionStateId( MiniTransactionStateConstant.PIN_SENT ) ;
						...
						response = this.getMessageSource().getMessage("MINI.TopupInfo", new Object[]{mobileNoParam, miniTransactionModel.getBAMT(), 
...
//			}
		...
		catch (FrameworkCheckedException ex)
		{
			...
			{
				response = this.getMessageSource().getMessage("MINI.GeneralError", new Object[]{mobileNoParam}, null) ;
