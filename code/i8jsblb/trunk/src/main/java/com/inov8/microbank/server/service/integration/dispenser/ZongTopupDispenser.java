...
import com.inov8.microbank.server.service.integration.vo.ZongTopupVO;
...
public class ZongTopupDispenser extends BillPaymentProductDispenser {
    ...
    public ZongTopupDispenser(CommissionManager commissionManager,
    ...
    public WorkFlowWrapper doSale(WorkFlowWrapper workFlowWrapper)
        ...
        if (this.logger.isDebugEnabled()) {
            this.logger.debug("Inside doSale of ZongTopupDispenser");
        }
...
//        ZongIntegrationMessageVO zongIntegrationVO = new ZongIntegrationMessageVO(false, "1003", workFlowWrapper.getTransactionCodeModel().getCode());
        ZongTopupVO zongTopupVO = (ZongTopupVO) workFlowWrapper.getProductVO();
        Date today = new Date();

        zongIntegrationVO.setMsidn(zongTopupVO.getConsumerNo());
        ...
        try {
            ...
            if (switchController == null) {
                this.logger
                        .error("[ZongTopupDispenser.doSale] Could not connect to Switch. Throwing Exception for Logged in AppuserID:"
                                + ThreadLocalAppUser.getAppUserModel()
                                        .getAppUserId()
                                + ". Exception:"
                                + "ZongTopupDispenser - Service cannot be invoked.");
            ...
            this.logger
                    .info("[ZongTopupDispenser.doSale] Going to make Topup Transacin for Logged in AppuserID:"
                            + ThreadLocalAppUser.getAppUserModel()
                                    .getAppUserId());
            zongIntegrationVO = (BOSSVO) switchController
                    .topup(zongIntegrationVO);
            ...
            if ((zongIntegrationVO.getResponse().getResponseCode() != null)
                ...
                this.logger
                        .info("[ZongTopupDispenser.doSale] Logged in AppUserID:"
                                + ThreadLocalAppUser.getAppUserModel()
                                        .getAppUserId()
                ...
                this.logger
                        .error("[ZongTopupDispenser.doSale] Logged in AppUserID:"
                                + ThreadLocalAppUser.getAppUserModel()
                                        .getAppUserId()
            ...
            this.logger
                    .error("[ZongTopupDispenser.doSale] Exception occured while making Zont Topup Logged in AppUserID:"
                            + ThreadLocalAppUser.getAppUserModel()
                                    .getAppUserId()
                            + " Exception Msg:"
                            + e.getMessage());
            ...
            this.logger
                    .error("[ZongTopupDispenser.doSale] Exception occured for Logged in AppUserID:"
                            + ThreadLocalAppUser.getAppUserModel()
                                    .getAppUserId()
                            + " Exception Msg:"
                            + ex.getMessage());
    ...
    public WorkFlowWrapper rollback(WorkFlowWrapper workFlowWrapper)
            throws FrameworkCheckedException {
        if (this.logger.isDebugEnabled()) {
            this.logger.debug("Inside Rollback of ZongTopupDispenser");
        }
        DateFormat dateFormat = new SimpleDateFormat("yyyyMMdd");
        DateFormat timeFormat = new SimpleDateFormat("HHmmss");
        BOSSVO zongIntegrationVO = new BOSSVO();
        ...
        try {
            ...
            zongIntegrationVO = (BOSSVO) switchController
                    .topupRollback(zongIntegrationVO);
            if (zongIntegrationVO != null) {
    ...
    private void parseResponse(String responseCode,
        ...
        if (this.logger.isDebugEnabled()) {
            this.logger.debug("Inside parseResponse of ZongTopupDispenser");
        }
        ...
        if ((list != null) && (list.size() > 0)) {
            ...
            this.logger
                    .debug("ZongTopupDispenser... NO MAPPING FOR ZONG PRODUCT IN PRODUCT_DEVICE_FLOW");
        }
