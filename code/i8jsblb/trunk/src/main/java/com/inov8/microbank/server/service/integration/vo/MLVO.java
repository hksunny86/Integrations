...
public class MLVO extends DonationPaymentVO
{
    ...
    {
        if( baseWrapper.getObject( CommandFieldConstants.KEY_WALKIN_SENDER_MSISDN ) != null )
        {
            String mobileNo = baseWrapper.getObject( CommandFieldConstants.KEY_WALKIN_SENDER_MSISDN ).toString().trim();
            mobileNo = this.formatWithoutZero(mobileNo);
            ((MLVO)productVO).setConsumerNo( mobileNo );
        }
        ...
        {
              ((MLVO)productVO).setBillAmount( Integer.parseInt(baseWrapper.getObject( CommandFieldConstants.KEY_BILL_AMOUNT ).toString().trim()) );
              ((MLVO)productVO).setCurrentBillAmount( Double.parseDouble(baseWrapper.getObject( CommandFieldConstants.KEY_BILL_AMOUNT ).toString().trim()) );
        ...
        {   
            ...
            {
                ((MLVO)productVO).setCurrentBillAmount( Double.parseDouble(baseWrapper.getObject( CommandFieldConstants.KEY_TX_AMOUNT ).toString().trim()) );
                ((MLVO)productVO).setBillAmount(Double.parseDouble(baseWrapper.getObject( "TXAM").toString().trim()) );
            }
        }
        
        return (MLVO)productVO;
    }
    ...
    {
        if( ((MLVO)productVO).getConsumerNo() == null )
            throw new FrameworkCheckedException("Mobile number is not provided.");
        if( ((MLVO)productVO).getBillAmount() == null || ((MLVO)productVO).getBillAmount() == 0 )
