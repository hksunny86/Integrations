...
import com.inov8.microbank.server.service.commandmodule.TellerCashInCommand;

public class TellerCashInVO implements ProductVO {
	...
	public ProductVO populateVO(ProductVO productVO, BaseWrapper baseWrapper) {
		...
		BaseCommand command = new TellerCashInCommand();	

		TellerCashInVO tellerCashInVO = (TellerCashInVO) productVO;
		...
		tellerCashInVO.setProductId(productId);
		tellerCashInVO.setAccountId(accountId);
		tellerCashInVO.setDeviceTypeId(deviceTypeId);
		tellerCashInVO.setTxAmount(txAmount);
		
		return tellerCashInVO;
	}
