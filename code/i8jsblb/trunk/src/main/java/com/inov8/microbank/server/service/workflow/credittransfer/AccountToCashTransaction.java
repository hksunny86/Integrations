...
public class AccountToCashTransaction
...
public AccountToCashTransaction()
  {
  }
	...
	private WorkFlowWrapper getBillInfo(WorkFlowWrapper wrapper) throws Exception{
		...
		{
			logger.debug("Inside loadProductDispenser(WorkFlowWrapper wrapper) of AccountToCashTransaction...");
  ...
  public WorkFlowWrapper doCreditTransfer(WorkFlowWrapper wrapper) throws Exception
  {
		...
		logger.info("[AccountToCashTransaction.doCreditTransfer()] Going to transfer funds from Customer Account to Acc-to-Cash Sundry Account. Transaction ID: " + switchWrapper.getWorkFlowWrapper().getTransactionCodeModel().getCode() + " SenderSmartMoneyAccountId: " + wrapper.getSmartMoneyAccountModel().getSmartMoneyAccountId());
		...
		((AccountToCashVO)(wrapper.getProductVO())).setBalance(customerBalance);
...
//							(agentBalance == null)?0.0:agentBalance,
		...
		String customerSmsText = this.getMessageSource().getMessage(
				...
				new Object[] { 
						...
						PortalDateUtils.formatDate(wrapper.getTransactionModel().getCreatedOn(), PortalDateUtils.SHORT_DATE_FORMAT),
						Formatter.formatDouble(wrapper.getCommissionAmountsHolder().getTransactionProcessingAmount()),
						customerBalance,wrapper.getOneTimePin(),
						}, null);
		...
		logger.info("[AccountToCashTransaction.doCreditTransfer()] Going to Settle Commissions. Transaction ID: " + wrapper.getTransactionCodeModel().getCode() + " SenderSmartMoneyAccountId: " + wrapper.getSmartMoneyAccountModel().getSmartMoneyAccountId());
...
protected WorkFlowWrapper doValidate(WorkFlowWrapper wrapper) throws
  ...
  protected WorkFlowWrapper doPreStart(WorkFlowWrapper wrapper)throws Exception
  {
	  ...
	  wrapper.getCustomerModel().setCustomerId(customerAppUserModel.getCustomerId());
	  ...
	  customerSma.setPaymentModeId(PaymentModeConstantsInterface.BRANCHLESS_BANKING_ACCOUNT);
...
*/
  ...
  /**
	...
	public WorkFlowWrapper rollback(WorkFlowWrapper wrapper) throws Exception
	{
		...
		logger.info("[AccountToCashTransaction.rollback] Rolling back A2C transaction with  ID: " + code);
		/*try{
