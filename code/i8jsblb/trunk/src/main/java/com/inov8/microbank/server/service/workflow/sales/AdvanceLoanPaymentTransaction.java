...
public class AdvanceLoanPaymentTransaction extends SalesTransaction {
    ...
    public void validateCommission(CommissionWrapper commissionWrapper, WorkFlowWrapper workFlowWrapper) throws FrameworkCheckedException {
        if (logger.isDebugEnabled()) {
            logger.debug("Inside validateCommission of AdvanceLoanPaymentTransaction...");
        }
...
//			throw new WorkFlowException(WorkFlowErrorCodeConstants.OPERATOR_BANK_INFO_NULL);
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Ending doValidate(WorkFlowWrapper wrapper) of AdvanceLoanPaymentTransaction");
        }
        return wrapper;
    }
    ...
    protected WorkFlowWrapper doSale(WorkFlowWrapper wrapper) throws Exception {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Inside doSale(WorkFlowWrapper wrapper) of AdvanceLoanPaymentTransaction..");
        }
         ...
         */
        logger.info("OLA > SENDING Advance Loan PAYMENT TO OLA BANKING.");

        ProductModel productModel = wrapper.getProductModel();

        switchWrapper = olaVeriflyFinancialInstitution.advanceLoanPayment(switchWrapper);
