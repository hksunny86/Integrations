...
public class AgentInternetBillPaymentTransaction extends SalesTransaction {
	...
	public AgentInternetBillPaymentTransaction() {
	}
...
//			switchWrapper.getWorkFlowWrapper().setTransactionModel( transactionModelTemp ) ;
			...
			if ( (productDeviceFlowModel != null && productDeviceFlowModel.getDeviceFlowId() != null && 
					productDeviceFlowModel.getDeviceFlowId().longValue() == 17) || InternetCompanyEnum.contains(String.valueOf(wrapper.getProductModel().getProductId())))
			{
				System.out.println();
...
//		txModel.setCustomerIdCustomerModel(wrapper.getCustomerModel());
	...
	public WorkFlowWrapper rollback(WorkFlowWrapper wrapper) throws Exception
	{
		...
		logger.info("Rolling back transaction with ID : "+code);
		...
		if(null != wrapper.getFirstFTIntegrationVO() &&  ! isBillAdviceSent)
		{
			...
			rrn = rrnPrefix + rrn;
			rrnPrefix = null;
			logger.info("FT was done so trying to roll back with RRN  : "+rrn+" and trx code : "+code);
			...
			logger.info("Hitting Phoenix to rollback with RRN : "+rrn+" and trx code : "+code);
