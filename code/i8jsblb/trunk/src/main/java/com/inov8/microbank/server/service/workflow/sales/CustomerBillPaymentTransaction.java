...
public class CustomerBillPaymentTransaction extends SalesTransaction {
...
//			throw new WorkFlowException(WorkFlowErrorCodeConstants.OPERATOR_BANK_INFO_NULL);
	...
	////***************************************************************************
		...
		if(logger.isDebugEnabled())
		{
			logger.debug("Ending doValidate(WorkFlowWrapper wrapper) of CustomerBillPaymentTransaction");
		}
		return wrapper;
...
//		this.validateCommission(commissionWrapper, _workFlowWrapper);
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Inside doSale(WorkFlowWrapper wrapper) of CustomerBillPaymentTransaction..");
		}
...
/***********/// need to prepare productVO from infoCommand
		...
		BillPaymentProductDispenser billSaleProductDispenser = null;

		OfflineBillersConfigModel offlineBillersConfigModel = getCommonCommandManager().loadOfflineBillersModelByProductId
				(String.valueOf(_workFlowWrapper.getProductModel().getProductId()));

		if(offlineBillersConfigModel == null) {
			billSaleProductDispenser = (BillPaymentProductDispenser) this.productDispenseController.loadProductDispenser(_workFlowWrapper);
		}
		...
		if(offlineBillersConfigModel != null){
//			switchWrapper.setIsOfflineBiller(true);
			_workFlowWrapper = doBillPaymentWithoutQueue(_workFlowWrapper);

//			TransactionCodeModel transactionCodeModel = transactionCodeDAO.findByPrimaryKey(transactionModelTemp.getTransactionCodeId());

			logger.info("Start Transaction of Offline Biller: "+ _workFlowWrapper.getProductModel().getProductId());
			I8SBSwitchControllerRequestVO requestVO = new I8SBSwitchControllerRequestVO();
			I8SBSwitchControllerResponseVO responseVO = new I8SBSwitchControllerResponseVO();
			requestVO = ESBAdapter.prepareOfflineBillerPaymentRequest(_workFlowWrapper.getConsumerNumber(),productModel);
			requestVO.setTransactionCode(_workFlowWrapper.getTransactionCodeModel().getCode());
			requestVO.setRefrenceNumber(_workFlowWrapper.getConsumerNumber());
			requestVO.setProductCode(_workFlowWrapper.getProductModel().getProductCode());
			switchWrapper.setI8SBSwitchControllerRequestVO(requestVO);
 			switchWrapper = getCommonCommandManager().makeI8SBCall(switchWrapper);
...
//			responseVO.setResponseCode("I8SB-200");
		...
		// for day end O.F. settlement of Core FT
		_workFlowWrapper.setTransactionModel(transactionModel);			
		...
		if (logger.isDebugEnabled()) {
			...
			logger.debug("Ending doSale of CustomerBillPaymentTransaction.");
		}
...
//			txManager.saveTransaction(workFlowWrapper);		
	...
	protected WorkFlowWrapper doBillPaymentWithoutQueue(WorkFlowWrapper workFlowWrapper) throws FrameworkCheckedException {
		...
		switchWrapper.setConsumerNumber(workFlowWrapper.getConsumerNumber());
