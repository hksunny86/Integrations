...
public class EnableDisableAgentWebPortalController extends AjaxController{
	...
	public String getResponseContent(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try{
			...
			Boolean isEnabled = new Boolean(ServletRequestUtils.getStringParameter(request, "isEnabledAgentWeb"));
			...
			if(null != retailerContactId && appUserModel.getAppUserTypeId().longValue() == 3){
				...
				retailerModel.setIsAgentWebEnabled(isEnabled);
				...
				if(isEnabled == true){
					buffer.append("Agent Web Portal has been enabled");
				}else{
					buffer.append("Agent Web Portal has been Disabled");
				}
				...
				if(null==handlerModel){
					throw new FrameworkCheckedException("Hander not found.");
				}
				
				handlerModel.setIsAgentWebEnabled(isEnabled);
				baseWrapper.setBasePersistableModel(handlerModel);					
