...
public class AllPayRegHeadSummaryReportController 
extends BaseFormSearchController
{

	private ReferenceDataManager referenceDataManager;
	private AllPayReportsManager allpayReportsManager;
	public AllPayRegHeadSummaryReportController() {
//		super.setFilterSearchCommandClass(RegionalHeadSummaryViewModel.class);
				
		super.setCommandName("allpayRetTransViewModel");
		super.setCommandClass(RegionalHeadSummaryViewModel.class);
...
//	}
	...
	protected ModelAndView onSearch(HttpServletRequest request, HttpServletResponse response, Object model, PagingHelperModel pagingHelperModel,LinkedHashMap<String, SortingOrder> sortingOrderMap) throws Exception 
	{
		...
		RegionalHeadSummaryViewModel regionalHeadSummaryViewModel = (RegionalHeadSummaryViewModel) model;
		
		regionalHeadSummaryViewModel.setRegionalHeadId( UserUtils.getCurrentUser().getRetailerContactId() ) ;
		//allpayRetTransViewModel.setRetailerContactId(getRetailerContactId());

		searchBaseWrapper.setBasePersistableModel(regionalHeadSummaryViewModel);
		...
		Date startDate = (Date)format.parse( "10/09/2009" );
		
//		regionalHeadSummaryViewModel.setStartDate( format.format(startDate) );
//		regionalHeadSummaryViewModel.setEndDate( format.format(startDate) );
...
//		searchBaseWrapper.setDateRangeHolderModel(dateRangeHolderModel);
		...
		searchBaseWrapper = this.allpayReportsManager.searchRegionalHeadSummary(searchBaseWrapper);
