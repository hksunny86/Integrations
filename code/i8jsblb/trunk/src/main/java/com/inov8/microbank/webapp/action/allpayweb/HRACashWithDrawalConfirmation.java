...
public class HRACashWithDrawalConfirmation extends AdvanceFormController {
    ...
    public HRACashWithDrawalConfirmation()
    {
    ...
    protected ModelAndView onCreate(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, Object o, BindException e) throws Exception {
        ...
        String PID = ProductConstantsInterface.HRA_CASH_WITHDRAWAL.toString();//requestWrapper.getParameter(PRODUCT_ID);
        ...
        requestWrapper.addParameter(CommandFieldConstants.KEY_CURR_COMMAND_ID,CommandFieldConstants.CMD_CASH_OUT_INFO);
        ...
        if(isOTPValidated != null && !isOTPValidated.equals(""))
        ...
        ThreadLocalUserDeviceAccounts.setUserDeviceAccountsModel(uda);
        requestWrapper.addParameter(CommandFieldConstants.KEY_DEVICE_TYPE_ID,DeviceTypeConstantsInterface.ALLPAY_WEB.toString());
        responseXML = mfsWebController.handleRequest(requestWrapper, CommandFieldConstants.KEY_CMD_HRA_CASH_WITHDRAWAL);
        if(responseXML != null && MfsWebUtil.isErrorXML(responseXML) ) {
