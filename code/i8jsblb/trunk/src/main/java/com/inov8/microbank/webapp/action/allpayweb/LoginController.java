...
public class LoginController extends AdvanceFormController {
...
//			STATUS = isPinChangeRequired(userDeviceAccountsModel, requestWrapper);
		...
		if(STATUS == SUCCESS_CASE || STATUS == PIN_CHANGE_REQUIRED || STATUS == TBR_REQUIRED || isTillBalanceRequired || appUserModel.getPasswordChangeRequired()) {
			...
			categories.remove(new Category(113));
			//categories.remove(new Category(114)); HRA Cash WithDrawal added on Agent Web
			categories.remove(new Category(115));
			categories.remove(new Category(116));
			categories.remove(new Category(120));
			categories.remove(new Category(44));
            categories.remove(new Category(119));
			categories.remove(new Category(47));//Old Challan Collection category
			categories.remove(new Category(122));//Customer Challan Collection category
			//End
...
*/
	...
	private Integer isPinChangeRequired(String responseXml, AllPayRequestWrapper requestWrapper) {
		...
		if(requestWrapper.getAttribute("IS_MIGRATED").equals("1"))
		{
			requestWrapper.getSession().setAttribute("IS_MIGRATED", "1" );
