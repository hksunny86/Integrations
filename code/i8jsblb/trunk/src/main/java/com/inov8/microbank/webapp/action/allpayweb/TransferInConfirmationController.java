...
public class TransferInConfirmationController extends AdvanceFormController {
	...
	public TransferInConfirmationController () {
	...
	protected ModelAndView onCreate(HttpServletRequest request, HttpServletResponse response, Object model, BindException exception) throws Exception {	
		...
		String responseXML = mfsWebController.handleRequest(requestWrapper, CommandFieldConstants.CMD_TRANSFER_IN);
		...
		if(!hasError) {
			...
			requestWrapper.setAttribute(AllPayWebConstant.HEADING.getValue(), "Transfer In");	
		}
	...
	private Boolean verifyPIN(AppUserModel appUserModel, String bankPin, Boolean fetchTitle, Long productId, HttpServletRequest request) {
		...
		try {
			...
			logger.info("[TransferInConfirmationController.verifyPin] Verifying Pin for AppUserID:" + appUserModel.getAppUserId() + ". Logged in AppUserID:" + ThreadLocalAppUser.getAppUserModel().getAppUserId());
			...
			logger.info("[TransferInConfirmationController.verifyPin] Exceptino occured in Verifying Pin for AppUserID:" + appUserModel.getAppUserId() + ". Logged in AppUserID:" + ThreadLocalAppUser.getAppUserModel().getAppUserId() + ". Exception Msg:" + e.getMessage());
	...
	private void updateRequestForPreviousScreen(AllPayRequestWrapper requestWrapper) throws Exception {
		...
		requestWrapper.addParameter(CommandFieldConstants.KEY_PROD_ID, String.valueOf(TransactionProductEnum.TRANSACTION_IN_PRODUCT.getProductId()));
		...
		requestWrapper.setAttribute(CommandFieldConstants.KEY_NAME, "Transfer In");
