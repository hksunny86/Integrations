...
public class UBPServiceStepOneController extends AdvanceFormController {
	...
	public UBPServiceStepOneController (){
	...
	protected ModelAndView onCreate(HttpServletRequest request, HttpServletResponse response, Object model, BindException exception) throws Exception {
		...
		String TAMTF = MiniXMLUtil.getTagTextValue(responseXml, "/msg/params/param[8]");
		String BDATE = MiniXMLUtil.getTagTextValue(responseXml, "/msg/params/param[13]");			
		
	    if((BDATE == null) || ("null".equalsIgnoreCase(BDATE.trim())) || "".equals(BDATE)) {
		...
		requestWrapper.addParameter("BDATE", BDATE);
		requestWrapper.setAttribute("BDATE", BDATE);		
		requestWrapper.addParameter("TAMTF", TAMTF);		
