...
public class SearchUserInfoController extends BaseFormSearchController{
	...
	/**
	 * This Method Searches the User Information (the MWallet User)
	 * Only activated users, i.e where USER_DEVICE_ACCOUNTS.IS_ACCOUNT_ENABLED = 1
	 */
	@Override
	protected ModelAndView onSearch(HttpServletRequest req, HttpServletResponse res,
									Object model, PagingHelperModel pagingHelperModel,
...
//		userInfoListViewModel.setAccountEnabled(true);	// to fix bug # 149
		...
		DateRangeHolderModel dateRangeHolderModel = new DateRangeHolderModel(
				"accountOpeningDate", userInfoListViewModel.getCreatedStartDate(),
				userInfoListViewModel.getCreatedEndDate());
		DateRangeHolderModel dateRangeHolderModelOnUpdateDate = new DateRangeHolderModel("accountUpdatedOn", userInfoListViewModel.getStartDate(),userInfoListViewModel.getEndDate());
	 ...
	 * @param resultList
	 */
	private void checkNameDuplication(List<UserInfoListViewModel> resultList) {

		for(UserInfoListViewModel viewModel : resultList) {
