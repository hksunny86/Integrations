 ...
 * Comments      : <br>
 */
public class AgentTransactionDetailViewController extends BaseFormSearchController
{
    ...
    public AgentTransactionDetailViewController()
    {
        setCommandClass( AgentTransactionDetailViewModel.class );
        setCommandName( "agentTransactionDetailViewModel" );
    }
    ...
    protected Map<String,Object> loadReferenceData( HttpServletRequest request ) throws Exception
    {
        ...
        try
        {
            ...
            if(!(UserUtils.getCurrentUser().getAppUserTypeId().equals(7L))) {
                ...
                Long soId = UserUtils.getCurrentUser().getMnoUserIdMnoUserModel().getMnoId();
                distributorModelList = findDistributorByserviceOperatorId( soId );
            }
                                     ...
                                     PagingHelperModel pagingHelperModel, LinkedHashMap<String, SortingOrder> sortingOrderMap ) throws Exception
    {
        ...
        AgentTransactionDetailViewModel agentTransactionDetailViewModel = ( (AgentTransactionDetailViewModel) model );

        if(UserUtils.getCurrentUser().getAppUserTypeId().equals(7L)){

            agentTransactionDetailViewModel.setSoId(UserUtils.getCurrentUser().getMnoUserIdMnoUserModel().getMnoId());
        }

        wrapper.setBasePersistableModel( agentTransactionDetailViewModel );
        ...
        DateRangeHolderModel dateRangeHolderModel = new DateRangeHolderModel( "transactionDate", 
            agentTransactionDetailViewModel.getStartDate(), agentTransactionDetailViewModel.getEndDate());
        ...
        wrapper = agentReportsFacade.searchAgentTransactionDetailView( wrapper );

        CustomList<AgentTransactionDetailViewModel> customList = wrapper.getCustomList();
        List<AgentTransactionDetailViewModel> agentTransactionDetailViewModelList = null;
        if( customList != null )
        {
            agentTransactionDetailViewModelList = customList.getResultsetList();
        }

        return new ModelAndView( getFormView(), "agentTransactionDetailViewModelList", agentTransactionDetailViewModelList );
    }

    private List<DistributorModel> findDistributorByserviceOperatorId( Long soId ) throws FrameworkCheckedException
    {
        List<DistributorModel> regionModelList = null;
            SearchBaseWrapper wrapper = agentHierarchyManager.findDistributorByserviceOperatorId( soId );

            CustomList<DistributorModel> customList = wrapper.getCustomList();
            if( customList != null )
            {
