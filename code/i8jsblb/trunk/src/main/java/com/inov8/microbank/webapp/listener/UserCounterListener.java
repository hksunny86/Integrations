    ...
    implements ServletContextListener, HttpSessionAttributeListener
{
  ...
  public void attributeAdded(HttpSessionBindingEvent event)
  {
    ...
    if (EVENT_KEY.equals(event.getName()) && !isAnonymous())
    {
      SecurityContext securityContext = (SecurityContext) event.getValue();
      AppUserModel user = (AppUserModel) securityContext.getAuthentication().
          getPrincipal();
      addUsername(user);
    }
  }
  ...
  public void attributeRemoved(HttpSessionBindingEvent event)
  {
    if (EVENT_KEY.equals(event.getName()) && !isAnonymous())
    {
      ...
      if (auth != null && (auth.getPrincipal() instanceof AppUserModel))
      {
        AppUserModel user = (AppUserModel) auth.getPrincipal();
        removeUsername(user);
      }
    }
  }
