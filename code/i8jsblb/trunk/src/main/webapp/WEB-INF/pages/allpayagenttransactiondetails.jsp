...
<meta name="decorator" content="decorator">
   ...
   <script type="text/javascript" src="<c:url value='/scripts/prototype.js'/>"></script>
	...
	<meta name="title" content="Agent Transaction History" />
  ...
  <script language="javascript" type="text/javascript">
	...
	<%
		...
		quickAgentTxHistPermission += "," + PortalConstants.HOME_PAGE_QUICK_AGNT_TX_HIST_READ;
	%>
  </head>
  ...
  <body bgcolor="#ffffff" onunload="javascript:closeChild();">
	...
	<table width="750px" border="0">
			<html:form name='customertransactionForm'
				commandName="extendedAllPayTransactionInfoListViewModel" method="post"
				action="allpayagenttransactiondetails.html" onsubmit="return validateForm(this)">
				<tr>
					...
					<td align="left" >
						<html:input path="recipientMobileNo" id="recipientMobileNo" cssClass="textBox" maxlength="11" tabindex="2" onkeypress="return maskNumber(this,event)"/> 
					</td>
				</tr>		
				<tr>
					<td class="formText" align="right">
				...
				<tr>
					...
					<td align="left">
						...
						<input name="reset" type="reset" 
							onclick="javascript: window.location='allpayagenttransactiondetails.html?agentId=${param.agentId}&retailerContactId=${param.retailerContactId}&appUserId=${param.appUserId}&actionId=2&referrer=${param.referrer}'"
				...
				<input type="hidden" name="retailerContactId" value="${param.retailerContactId}">
...
<ec:table filterable="false"
		items="transactionDetInfoListViewModelList"
                var="transactionDetInfoListViewModel"
                retrieveRowsCallback="limit"
                filterRowsCallback="limit"
                sortRowsCallback="limit"
		action="${pageContext.request.contextPath}/allpayagenttransactiondetails.html?actionId=2&agentId=${param.agentId}"
		title="">
		...
		<authz:authorize ifAnyGranted="<%=PortalConstants.EXPORT_PDF_READ%>">
			<ec:exportPdf headerBackgroundColor="#b6c2da" headerTitle="Agent Transactions"
				fileName="Agent Transactions.pdf" tooltip="Export PDF" view="com.inov8.microbank.common.util.CustomPdfView" />
		</authz:authorize>
		...
		<ec:row>
			...
			<ec:column property="serviceChargesExclusive" filterable="false" title="Service Charges" calc="total" calcTitle="Total:"  cell="currency"  format="0.00" style="text-align: right" />
			<ec:column property="totalChargedFromCustomer" filterable="false" title="Total Charged From Customer" calc="total" calcTitle="Total:"  cell="currency"  format="0.00" style="text-align: right"/>
				...
				<authz:authorize ifAnyGranted="<%=smsPermission%>">
					<c:choose>
						<c:when test="${transactionDetInfoListViewModel.initiatorResendSmsAvailable}">
							<input type="button" class="button" value="${transactionDetInfoListViewModel.initiatorResendSmsBtnLabel}" id="initiatorSms${transactionId}" onclick="openResendSmsWindow('${transactionId}','${INITIATOR_SMS_STRATEGY}','${param.appUserId}','${transactionDetInfoListViewModel.transactionCode}')"/>
			<!--  				<ajax:htmlContent baseUrl="${contextPath}/p_resendSmsUsingStrategy.html" source="initiatorSms${transactionId}"
								target="smsContent" parameters="tId=${transactionId},resendSmsStrategy=${INITIATOR_SMS_STRATEGY},appUserId=${param.appUserId}" postFunction="handleResendSmsResponse"/>
			-->
						</c:when>
						<c:otherwise>
				...
				<authz:authorize ifAnyGranted="<%=smsPermission%>">
					<c:choose>
						<c:when test="${transactionDetInfoListViewModel.recipientResendSmsAvailable}">
							<input type="button" class="button" value="${transactionDetInfoListViewModel.recipientResendSmsBtnLabel}" id="recipientSms${transactionId}" onclick="openResendSmsWindow('${transactionId}','${RECIPIENT_SMS_STRATEGY}','${param.appUserId}','${transactionDetInfoListViewModel.transactionCode}')"/>
			<!--  				<ajax:htmlContent baseUrl="${contextPath}/p_resendSmsUsingStrategy.html" source="recipientSms${transactionId}"
								target="smsContent" parameters="tId=${transactionId},resendSmsStrategy=${RECIPIENT_SMS_STRATEGY},appUserId=${param.appUserId}" postFunction="handleResendSmsResponse"/>
					-->
						</c:when>
						<c:otherwise>
							<input type="button" class="button" value="N/A" disabled="disabled"/>
				...
				<authz:authorize ifAnyGranted="<%=smsPermission%>">
					<c:choose>
						<c:when test="${transactionDetInfoListViewModel.walkinDepositorSmsAvailable}">
							<input type="button" class="button" value="Depositor" id="walkInDepositorSms${transactionId}" onclick="openResendSmsWindow('${transactionId}','${WALKIN_DEPOSITOR_SMS_STRATEGY}','${param.appUserId}','${transactionDetInfoListViewModel.transactionCode}')"/>
			<!-- 				<ajax:htmlContent baseUrl="${contextPath}/p_resendSmsUsingStrategy.html" source="walkInDepositorSms${transactionId}"
								target="smsContent" parameters="tId=${transactionId},resendSmsStrategy=${WALKIN_DEPOSITOR_SMS_STRATEGY},appUserId=${param.appUserId}" postFunction="handleResendSmsResponse"/>
				 -->
						</c:when>
						<c:otherwise>
							<input type="button" class="button" value="N/A" disabled="disabled"/>
				...
				<authz:authorize ifAnyGranted="<%=smsPermission%>">
					<c:choose>
						<c:when test="${transactionDetInfoListViewModel.walkinBeneficiarySmsAvailable}">
							<input type="button" class="button" value="Beneficiary" id="walkInBeneficiarySms${transactionId}" onclick="openResendSmsWindow('${transactionId}','${WALKIN_BENEFICIARY_SMS_STRATEGY}','${param.appUserId}','${transactionDetInfoListViewModel.transactionCode}')"/>
					<!--  		<ajax:htmlContent baseUrl="${contextPath}/p_resendSmsUsingStrategy.html" source="walkInBeneficiarySms${transactionId}"
								target="smsContent" parameters="tId=${transactionId},resendSmsStrategy=${WALKIN_BENEFICIARY_SMS_STRATEGY},appUserId=${param.appUserId}" postFunction="handleResendSmsResponse"/>
					-->
						</c:when>
						<c:otherwise>
							<input type="button" class="button" value="N/A" disabled="disabled"/>
   	...
   	<c:if test="${empty param.referrer}">
   		<input type="button" class="button" value="Back" tabindex="48" onclick="javascript: window.location.href='home.html'"/>
   	</c:if>
   	<c:if test="${not empty param.referrer}">
   	<input type="button" class="button" value="Back" tabindex="48" onclick="javascript: window.location.href='allpayRetailerAccountDetails.html?referrer=${param.referrer}&retailerContactId=${param.retailerContactId}&appUserId=${param.appUserId}'" />
	</c:if>
