...
<html>
  <head>
		...
		<meta name="title" content="Bulk Update Customer Screening Performed" />
  ...
  <body bgcolor="#ffffff">
    ...
    <c:if test="${not empty messages}">
          ...
          <c:if test="${not empty bulkUpdateCustomerScreeningModelsList}">
			<html:form name="bulkScreeningPerformForm" commandName="bulkUpdateCustomerScreeningModel" method="post" action="p-updatecustomerscreening.html">
				<table width="750px" border="0">
				   <c:if test="${totalInValidRecords > 0}">
					 ...
					 <tr>
					   <td class="formText">
						 <a href="${contextPath}/p_downloadInvalidCustomerBulkUpdateScreeningPerformedRecordsFile.html">Export Invalid Records</a>
					   </td>
					</tr>	
	             </c:if>
				 <c:if test="${totalValidRecords > 0}">
					<tr>
					 <td class="formText">
					   <input id="_save" type="button" name="_save" value="  Mark Screening " tabindex="1" onclick="javascript:disableSubmit();"class="button" />
					   &nbsp;&nbsp;&nbsp;&nbsp;
					   <input type="button" class="button" value="Cancel" tabindex="2" onClick="javascript: window.location='p-updatecustomerscreening.html'" />
					</td>
				  </tr>
			  </c:if>
		</table>
		...
		<ec:table items="bulkUpdateCustomerScreeningModelsList" var="bulkScreeningModel" action="${contextPath}/p-updatecustomerscreeningpreview.html"
			title="" retrieveRowsCallback="limit" filterRowsCallback="limit" sortRowsCallback="limit" filterable="false" sortable="false">
			<ec:row interceptor="bulkScreeningPerformedInvalidRowInterceptor">
				<ec:column property="srNo" title="Sr #"/>
				<ec:column property="screeningPerformed"/>
				<ec:column property="cnic" title="CNIC"/>
				<ec:column property="description" title="Error Reason"/>
			</ec:row>
		</ec:table>

	<input type="button" class="button" value="Back" tabindex="3" onclick="javascript: window.location.href='p-updatecustomerscreening.html?actionId=1'" />
		
		
		<script language="javascript" type="text/javascript">
			function disableSubmit()
		    {
		    	document.forms.bulkScreeningPerformForm._save.disabled=true;
		    	
			    onSave(document.forms.bulkScreeningPerformForm,null);
		    }
		</script>
