...
<head>
    ...
    <meta name="title" content="BDE KPI Report" />
...
<html:form name="bdeKpiReportViewForm" commandName="bdeKpiReportViewModel" method="post"
           action="p_bdekpireport.html" onsubmit="return validateForm(this)">
        ...
        <tr>
            ...
            <td align="left">
                ...
                <input name="reset" type="reset"
                       onclick="javascript: window.location='p_bdekpireport.html?actionId=${retriveAction}'"
...
<ec:table items="bdeKpiReportViewModelList" var="bdeKpiReportViewModel"
          retrieveRowsCallback="limit" filterRowsCallback="limit"
          sortRowsCallback="limit" action="${contextPath}/p_bdekpireport.html?actionId=${retriveAction}"
          title="" width="100%" filterable="false" sortable="true">

    <authz:authorize ifAnyGranted="<%=PortalConstants.PERMS_EXPORT_XLS_READ%>">
        <ec:exportXls fileName="BDE KPI Report.xls" tooltip="Export Excel" />
    </authz:authorize>
    <authz:authorize ifAnyGranted="<%=PortalConstants.PERMS_EXPORT_XLSX_READ%>">
        <ec:exportXlsx fileName="BDE KPI Report.xlsx" tooltip="Export Excel" />
    </authz:authorize>
    <authz:authorize ifAnyGranted="<%=PortalConstants.PERMS_EXPORT_PDF_READ%>">
        <ec:exportPdf view="com.inov8.microbank.common.util.CustomPdfView" headerBackgroundColor="#b6c2da"
                      headerTitle="BDE KPI Report" fileName="BDE KPI Report.pdf" tooltip="Export PDF" />
    </authz:authorize>
    <authz:authorize ifAnyGranted="<%=PortalConstants.PERMS_EXPORT_CSV_READ%>">
        <ec:exportCsv fileName="BDE KPI Report.csv" tooltip="Export CSV"></ec:exportCsv>
    </authz:authorize>
    <ec:row>
<%--        <c:set var="appUserId">--%>
...
<%--        <ec:column title="Core Ac #" property="coreAccountNo"/>--%>
        ...
        <ec:column title="BDE Employee ID" property="bdeEmployeeId"/>
        <ec:column title="BDE Name" property="bdeName"/>
        <ec:column title="Designation" property="designation"/>
        <ec:column title="ITP Challan Transaction" property="itpChallanTransactions"/>
        <ec:column title="Total ITP Challan Transactions Amount" property="itpChallanTransactionsAmount"/>
        <ec:column title="Wallet Opening" property="walletOpening"/>
        <ec:column title="UBP Transaction" property="ubpTransactions"/>
        <ec:column title="Total UBP Transactions Amount" property="ubpTransactionsAmount"/>
        <ec:column title="KPK Challan Transaction" property="kpkChallanTransactions"/>
        <ec:column title="Total KPK Challan Transactions Amount" property="kpkChallanTransactionsAmount"/>
        <ec:column title="Transaction Date" property="transactionSummaryDate"/>
