...
<html>
   <head>
		...
		<meta name="title" content="Bulk Manual Adjustment" />
	...
	<body bgcolor="#ffffff">
		<c:if test="${not empty messages}">
          ...
          <c:if test="${not empty bulkManualAdjustmentsModelList}">
			<html:form name="bulkAdjustmentForm" commandName="bulkManualAdjustmentModel" method="post" action="p_bulkmanualadjustment.html">
				<input type="hidden" name="usecaseId" value = "<%=PortalConstants.BULK_MANUAL_ADJUSTMENT_USECASE_ID %>"/>
				...
				<table width="750px" border="0">
				   <c:if test="${totalInValidTraxns > 0}">
					 ...
					 <tr>
					   <td class="formText">
						 <a href="${contextPath}/p_downloadinvalidbulkadjustmentrecordsfile.html">Export Invalid Records</a>
					   </td>
					</tr>	
	             </c:if>
				 <c:if test="${totalValidTraxns > 0}">
					<tr>
					 <td class="formText">
					   ...
					   &nbsp;&nbsp;&nbsp;&nbsp;
					   <input type="button" class="button" value="Cancel" tabindex="2" onClick="javascript: window.location='p_bulkmanualadjustment.html'" />
					</td>
				  </tr>
			  </c:if>
		</table>
		...
		<ec:table items="bulkManualAdjustmentsModelList" var="bulkManualAdjustmentModel" action="${contextPath}/p_bulkmanualadjustmentpreview.html"
			title="" retrieveRowsCallback="limit" filterRowsCallback="limit" sortRowsCallback="limit" filterable="false" sortable="false">
			<ec:row interceptor="bulkManualAdjustmentErrorRowInterceptor">
				<ec:column property="srNo"/>
				<ec:column alias="Adjustment Type">
				<c:if test="${bulkManualAdjustmentModel.adjustmentType == 1}">
					BB to BB
				</c:if>
				<c:if test="${bulkManualAdjustmentModel.adjustmentType == 2}">
					BB to Core
				</c:if>
				<c:if test="${bulkManualAdjustmentModel.adjustmentType == 3}">
					Core to BB
				</c:if>
				<c:if test="${bulkManualAdjustmentModel.adjustmentType > 3 or bulkManualAdjustmentModel.adjustmentType < 1 }">
					${bulkManualAdjustmentModel.adjustmentType} 
				</c:if>
				</ec:column>
	...
	<input type="button" class="button" value="Back" tabindex="3" onclick="javascript: window.location.href='p_bulkmanualadjustment.html?actionId=1'" />
		
		
		<script language="javascript" type="text/javascript">
			function disableSubmit()
		    {
		    	document.forms.bulkAdjustmentForm._save.disabled=true;
		    	
			    onSave(document.forms.bulkAdjustmentForm,null);
		    }
		</script>
  </body>
</html>
