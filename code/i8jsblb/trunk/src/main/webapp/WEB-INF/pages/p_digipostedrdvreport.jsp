...
<head>
    ...
    <meta name="title" content="Digi Posted RDV Report"/>
...
<html:form name="digipostedrdvviewForm" commandName="DigiPostedRdvReportController" method="post"
           action="${contextPath}/p_digipostedrdvreport.html">
        ...
        <tr>
            <td align="left">
                ...
                <input name="reset" type="reset"
                       onclick="javascript: window.location='p_digipostedrdvreport.html?actionId=${retriveAction}'"
...
<ec:table items="reqList" var="model"
          ...
          sortRowsCallback="limit" action="${contextPath}/p_digipostedrdvreport.html?actionId=${retriveAction}"
          title="" width="100%" filterable="false" sortable="true">

    <authz:authorize ifAnyGranted="<%=PortalConstants.PERMS_EXPORT_XLS_READ%>">
        <ec:exportXls fileName="Digi Posted RDV Report.xls" tooltip="Export Excel"/>
    </authz:authorize>
    <authz:authorize ifAnyGranted="<%=PortalConstants.PERMS_EXPORT_XLSX_READ%>">
        <ec:exportXlsx fileName="Digi Posted RDVReport.xlsx" tooltip="Export Excel"/>
    </authz:authorize>
    <authz:authorize ifAnyGranted="<%=PortalConstants.PERMS_EXPORT_PDF_READ%>">
        <ec:exportPdf view="com.inov8.microbank.common.util.CustomPdfView" headerBackgroundColor="#b6c2da"
                      headerTitle="Digi Posted RDV Report" fileName="Digi Posted RDV Report.pdf" tooltip="Export PDF"/>
    </authz:authorize>
    <authz:authorize ifAnyGranted="<%=PortalConstants.PERMS_EXPORT_CSV_READ%>">
        <ec:exportCsv fileName="Digi Posted RDV Report.csv" tooltip="Export CSV"></ec:exportCsv>
    </authz:authorize>
    <ec:row>
        <%--        <c:set var="appUserId">--%>
        <%--            <security:encrypt strToEncrypt="${bdeKpiReportViewModel.appUserId}"></security:encrypt>--%>
        <%--        </c:set>--%>
        <ec:column title="Transaction ID" property="trnasactionId"/>
        <ec:column title="Transaction Date" property="transactionDate"/>
        <%--        <ec:column title="Core Ac #" property="coreAccountNo"/>--%>
        <ec:column title="Transaction Type" property="transactionType"/>
        <ec:column title="Product Name" property="productName"/>
        <ec:column title="RRN" property="rrn"/>
        <ec:column title="Consumer No" property="consumerNo"/>
        <ec:column title="From Mobile" property="fromMobile"/>
        <ec:column title="To Account" property="toAccount"/>
        <ec:column title="To Mobile" property="toMobile"/>
        <ec:column title="Amount" property="amount"/>
        <ec:column title="Response Code" property="responseCode"/>
        <ec:column title="Response Code Narration" property="responseCodeNarration"/>
        <ec:column title="Status" property="status"/>
        <ec:column title="Stan" property="stan"/>
