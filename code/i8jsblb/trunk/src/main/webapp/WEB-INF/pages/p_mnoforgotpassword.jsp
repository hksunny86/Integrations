...
<meta name="decorator" content="decorator">
		...
		<meta name="title" content="Forgot Customer Web Password" />
	...
	<body bgcolor="#ffffff">
		...
		<html:form name="mnoforgotPin" commandName="forgotpinListViewModel" onsubmit="return validateForm()" >
	  				...
	  				<tr>
	       				<td width="33%"  align="right" class="formText">Agent ID:</td>
					...
					<tr>
			          ...
			          <security:isauthorized action="<%=PortalConstants.ACTION_RETRIEVE%>">
					    ...
					    <input type="reset" tabindex="4" name="cancel" value="Cancel" class="button" onclick="javascript:window.location='p_mnoforgotpassword.html?actionId=${retriveAction}'"/></td>
						<td  align="left">&nbsp;</td>
						<td  align="left">&nbsp;
						</td>		
			...
			<ec:table filterable="false"
				...
				action="${pageContext.request.contextPath}/p_mnoforgotpassword.html?actionId=${retriveAction}"
				title="">
				<authz:authorize ifAnyGranted="<%=PortalConstants.PERMS_EXPORT_XLS_READ%>">
					<ec:exportXls fileName="Forgot Customer Web Password.xls" tooltip="Export Excel" />
				</authz:authorize>
				<authz:authorize ifAnyGranted="<%=PortalConstants.PERMS_EXPORT_XLSX_READ%>">
					<ec:exportXlsx fileName="Forgot Customer Web Password.xlsx" tooltip="Export Excel" />
				</authz:authorize>
				<authz:authorize ifAnyGranted="<%=PortalConstants.PERMS_EXPORT_PDF_READ%>">
					<ec:exportPdf view="com.inov8.microbank.common.util.CustomPdfView" headerBackgroundColor="#b6c2da"
						headerTitle="Forgot Customer Web Password" ileName="Forgot Customer Web Password.pdf" tooltip="Export PDF" />
				<authz:authorize ifAnyGranted="<%=PortalConstants.PERMS_EXPORT_CSV_READ%>">
					<ec:exportCsv fileName="Forgot Customer Web Password.csv" tooltip="Export CSV"></ec:exportCsv>
				</authz:authorize>
  				...
  				<ec:row>
    				...
    				<ec:column property="userId" filterable="false" title="Customer ID"/>
					...
					<input type="button"  class="button" value="         Locked        " id="chgPin${forgotPinAppUserId}" disabled="disabled"/>
						...
						<jsp:attribute name="ifAccessNotAllowed"> 
						 <input type="button"  class="button" value="Change Password" id="chgPin${forgotpinListAppUserId}" disabled="disabled"/>    
						</jsp:attribute>
					...
					<input type="hidden" value="${forgotpinListAppUserId}" name="appUser${forgotpinListAppUserId}" id="appUser${forgotpinListAppUserId}" />
						<input type="hidden" id="usecaseId${forgotpinListAppUserId}" value="<%=PortalConstants.MNO_FORGOT_PASSWORD_USECASE_ID%>"/>	
						<input type="hidden" id="actionId${forgotpinListAppUserId}" value="<%=PortalConstants.ACTION_UPDATE%>"/>
						<ajax:htmlContent baseUrl="${contextPath}/p_mnoforgotpasswordform.html" 
							eventType="click" 
