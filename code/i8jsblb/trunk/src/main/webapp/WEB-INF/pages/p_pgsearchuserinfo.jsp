...
<head>
	...
	<script type="text/javascript">
		...
		jq(document).ready(
				function($)
				{
					...
					if(${fn:length(userInfoListViewModelList)} < 1){
					$('#selectAll').hide();
				}else{
					$('#selectAll').show();
				}
	...
	<meta name="title" content="Search Customer"   id="<%=ReportIDTitleEnum.SearchCustomerReport.getId()%>" />
</head>
...
<html:form name="userInfoListViewForm"
		   commandName="userInfoListViewModel" onsubmit="return validateForm(this)" >
	<table width="950px">
		<tr>
		...
		<tr>
			...
			<td align="left" colspan="2" class="formText">
				...
				<input type="reset" class="button" value="Cancel" name="_reset" onclick="javascript: window.location='p_pgsearchuserinfo.html?actionId=${retriveAction}'" tabindex="7" />
...
<ec:table filterable="false" items="userInfoListViewModelList"
		  ...
		  action="${pageContext.request.contextPath}/p_pgsearchuserinfo.html?actionId=${retriveAction}"
		  title="">
	<authz:authorize ifAnyGranted="<%=PortalConstants.PERMS_EXPORT_XLS_READ%>">
		<ec:exportXls fileName="User Info List.xls" tooltip="Export Excel" />
	</authz:authorize>
	<authz:authorize ifAnyGranted="<%=PortalConstants.PERMS_EXPORT_XLSX_READ%>">
		<ec:exportXlsx fileName="User Info List.xlsx" tooltip="Export Excel" />
	</authz:authorize>
	...
	<authz:authorize ifAnyGranted="<%=PortalConstants.PERMS_EXPORT_CSV_READ%>">
		<ec:exportCsv fileName="User Info List.csv" tooltip="Export CSV"></ec:exportCsv>
	</authz:authorize>
	<ec:row>
		<c:set var="userInfoListViewAppUserId">
			<security:encrypt strToEncrypt="${userInfoListViewModel.appUserId}"/>
		</c:set>
		<ec:column title=" " sortable="false" property="appUserId" viewsAllowed="html">
			<input type="checkbox" class="checkbox"/>
			<input type="hidden" value="${userInfoListViewModel.appUserId}">
		</ec:column>
		...
		<ec:column property="nic" filterable="false" title="NIC #"  escapeAutoFormat="true">
			...
			<c:choose>
				<c:when test="${userInfoListViewModel.isAccountClosed =='CLOSED UNSETTLED' || userInfoListViewModel.isAccountClosed =='CLOSED SETTLED'}">
					${userInfoListViewModel.nic}
				</c:when>
				<c:otherwise>
					<a href="p_verisysdata.html?actionId=${retriveAction}&id=${userInfoListViewModel.nic}" onClick="return openVerisysWindow('${userInfoListViewModel.nic}')">
							${userInfoListViewModel.nic}
					</a>
				</c:otherwise>
			</c:choose>

			<%--		<c:if test="${userInfoListViewModel.isAccountClosed =='CLOSED UNSETTLED'}">
						${userInfoListViewModel.nic}
					</c:if>
					<c:if test="${userInfoListViewModel.isAccountClosed =='CLOSED SETTLED'}">
						${userInfoListViewModel.nic}
					</c:if>--%>
		...
		<ec:column alias=" " viewsAllowed="html" filterable="false" sortable="false">
			...
			<c:if test="${userInfoListViewModel.isAccountLocked=='LOCKED'}">
					...
					<c:choose>
						...
						<c:otherwise>
							<authz:authorize ifAnyGranted="<%=updateL2Permission%>">
								<input type="button" class="button" style="width:90px" value="Change Info"
									   onclick="javascript:changeInfo('${contextPath}/p_l2_kyc_form.html?appUserId=${userInfoListViewModel.appUserId}');" />
							</authz:authorize>
		...
		<c:if test="${isNotRetailer}">
			<ec:column alias=" " viewsAllowed="html" filterable="false" sortable="false">
				<c:choose>
					...
					<c:otherwise>
						<input type="button" class="button" style="width='90px'" value="View Details." onclick="javascript:window.location.href='${pageContext.request.contextPath}/p_level2accountdetails.html?appUserId=${userInfoListViewModel.appUserId}&actionId=2';" />
					</c:otherwise>
				</c:choose>
			</ec:column>
		</c:if>
