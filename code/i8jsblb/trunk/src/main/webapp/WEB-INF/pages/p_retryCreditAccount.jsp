...
<meta name="title" content="Retry Debit/Credit Queue Report" />
...
<body>
	...
	<html:form name="retryAdviceListViewSearchForm" commandName="retryCreditQueueSearchVO" method="post" action="p_retryCreditAccount.html"
		onsubmit="return validateForm(this);" >
		<table width="850px" border="0">
			
			<tr>
				<td class="formText" width="18%" align="right">Transaction ID:</td>
				<td align="left" width="32%"><html:input path="transactionCode" id="transactionCode" cssClass="textBox" maxlength="50" tabindex="1"
						onkeypress="return maskNumber(this,event)" />
				</td>
				<td class="formText" align="right">Status:</td>
				<td align="left">
					<html:select path="status" cssClass="textBox" tabindex="7">
			...
			<tr>
				<td class="formText" align="right">Transaction Type:</td>
				<td align="left" colspan="3">
					<html:select path="transactionTypeId" cssClass="textBox" tabindex="8">
						<html:option value="">---All---</html:option>
	...
	<!-- data table result -->
	<ec:table items="resultList" var="model" action="${contextPath}/p_retryCreditAccount.html" title="" retrieveRowsCallback="limit"
		...
		<ec:row>
			...
			<ec:column property="balance" title="Transaction Amount" escapeAutoFormat="true" cell="currency" format="0.00" style="text-align: right"/>
			<ec:column property="createdOn" title="Transmission Time" escapeAutoFormat="true" cell="date" format="dd/MM/yyyy hh:mm a" />
			<ec:column property="accountNumber" title="Account No" escapeAutoFormat="true" />
			<ec:column property="accountType" title="Account Type" escapeAutoFormat="true" />
			<ec:column property="accountHolderName" title="Account Title" escapeAutoFormat="true" />
				...
				<authz:authorize ifAnyGranted="<%=PortalConstants.RPT_RETRY_ADVICE_UPDATE%>">
					<c:choose>
						<c:when test="${model.status eq 'Failed'}">
					    	<input type="button" class="button" value="Retry" id="btnretry_${model.retryCreditQueueViewModelId}" onclick="disableRetryButton(this), deleteRow(this);"/>
						</c:when>
					    <c:otherwise>
					    	<input type="button" class="button" disabled="disabled" value="Retry" id="btnretry_${model.retryCreditQueueViewModelId}" />
					    </c:otherwise>
					</c:choose>
					<ajax:updateField baseUrl="${contextPath}/p_retryCreditAccountAjaxRequest.html" 
					source="btnretry_${model.retryCreditQueueViewModelId}" eventType="click"
					target="message" action="btnretry_${model.retryCreditQueueViewModelId}" 
					parameters="accountCreditQueueLogId=${model.retryCreditQueueViewModelId}"
