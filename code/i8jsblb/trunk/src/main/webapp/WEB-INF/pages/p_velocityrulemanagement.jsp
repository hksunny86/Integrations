...
<meta name="decorator" content="decorator">
	...
	<%
		String createPermission = PortalConstants.MNG_VELOCITY_RULES_CREATE;
		createPermission +=	"," + PortalConstants.PG_GP_CREATE;
		createPermission +=	"," + PortalConstants.ADMIN_GP_CREATE;

		String updatePermission =	PortalConstants.MNG_VELOCITY_RULES_UPDATE;
	...
	<body bgcolor="#ffffff" onunload="javascript:closeChild();">
		...
		<c:if test="${not empty messages}">
			...
			<html:form name='extendedVelocityRuleModelForm' commandName="extendedVelocityRuleModel" method="post"
				action="p_velocityrulemanagement.html" onsubmit="return validateForm(this)" >
				<table width="770px" border="0">
				<authz:authorize ifAnyGranted="<%=createPermission %>">
				<tr>
					<td colspan="4">
						<div align="left"><a href="p_velocityruleform.html?actionId=1" class="linktext">Velocity Rules </a><br>&nbsp;&nbsp;</div>	
					</td>		
				</tr>
				</authz:authorize>	
				<tr>
				...
				<tr>
					...
					<td align="left">
						...
						<input name="reset" type="reset"
							onclick="javascript: window.location='p_velocityrulemanagement.html?actionId=${retriveAction}'"
		...
		<ec:table items="velocitRuleModelList" var="velocitRuleModel"
		action="${contextPath}/p_velocityrulemanagement.html"
		title="" retrieveRowsCallback="limit" filterRowsCallback="limit" sortRowsCallback="limit" filterable="false">
			<ec:row>	
				...
				<ec:column property="limitTypeIdLimitTypeModel.name" title="Limit Type" escapeAutoFormat="True" style="text-align: center"/>
				...
				<ec:column property="isActive" title="Status" escapeAutoFormat="true" style="text-align: center">			
					<c:if test="${velocitRuleModel.isActive == true}">Active </c:if>
					<c:if test="${velocitRuleModel.isActive != true}">Deactivate </c:if>  
				</ec:column>
				<ec:column alias="" title="" style="text-align: center" filterable="false" sortable="false" viewsAllowed="html">
					<authz:authorize ifAnyGranted="<%=updatePermission %>">			
						<input type="button" class="button" value="Edit" 
							onclick="javascript:document.location='p_velocityruleform.html?velocityRuleId=${velocitRuleModel.velocityRuleId}&distributorId=${velocitRuleModel.distributorId}'" />		
					</authz:authorize>
