...
<head>
	...
	<meta name="title" content="Cls Wallet account Opening Tracking Report" />
...
<html:form name="retryAdviceListViewSearchForm" commandName="clsPendingAccountOpeningModel" method="post" action="p_walletaccountopeningtrackingreports.html"
	...
	<table width="850px" border="0">
		...
		<tr>
			...
			<td align="left">
				<html:select path="name" cssClass="textBox">
					<html:option value="">[All]</html:option>
					<c:if test="${caseStatusModelList != null}">
						<html:options items="${caseStatusModelList}" itemLabel="name" itemValue="name" />
					</c:if>
				</html:select>
			</td>
		</tr>
...
<!-- data table result -->
<ec:table items="bookMeList" var="bookMeList" action="${contextPath}/p_walletaccountopeningreports.html" title="" retrieveRowsCallback="limit"
	...
	<ec:row>
		...
		<ec:column property="accountStateModel.name" title="Account State" escapeAutoFormat="true" />
		<ec:column property="consumerName" title="Customer Name" escapeAutoFormat="true" />
		<ec:column property="registrationStateModel.name" title="Registration State" escapeAutoFormat="true" />
