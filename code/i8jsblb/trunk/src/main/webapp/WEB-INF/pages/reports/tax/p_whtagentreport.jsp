...
<meta name="title" content="Agent WHT Report" />
...
<body>
	...
	<html:form name="wHTAgentReportForm" commandName="wHTAgentReportViewModel" method="post" action="p_whtagentreport.html"
		...
		<table width="850px" border="0">
			<tr>
				...
				<td align="left">
					<html:input tabindex="2" maxlength="15" path="cnic" cssClass="textBox"/>
				</td>
			</tr>
			<tr>
	...
	<!-- data table result -->
	<ec:table items="resultList" var="model" action="${contextPath}/p_whtagentreport.html" title="" retrieveRowsCallback="limit"
		filterRowsCallback="limit" sortRowsCallback="limit" filterable="false">
		<authz:authorize ifAnyGranted="<%=PortalConstants.PERMS_EXPORT_XLS_READ%>">
			<ec:exportXls fileName="Agent_WHT_Report.xls" tooltip="Export Excel" />
		</authz:authorize>
		<authz:authorize ifAnyGranted="<%=PortalConstants.PERMS_EXPORT_XLSX_READ%>">
			<ec:exportXlsx fileName="Agent_WHT_Report.xlsx" tooltip="Export Excel" />
		</authz:authorize>
		<authz:authorize ifAnyGranted="<%=PortalConstants.PERMS_EXPORT_PDF_READ%>">
			<ec:exportPdf view="com.inov8.microbank.common.util.CustomPdfView" headerBackgroundColor="#b6c2da" headerTitle="Agent WHT Report"
				fileName="Agent_WHT_Report.pdf" tooltip="Export PDF" />
		</authz:authorize>
		<authz:authorize ifAnyGranted="<%=PortalConstants.PERMS_EXPORT_CSV_READ%>">
			<ec:exportCsv fileName="Agent_WHT_Report.csv" tooltip="Export CSV"></ec:exportCsv>
		</authz:authorize>
		<ec:row>
			...
			<ec:column property="taxRate" title="Tax Rate" escapeAutoFormat="true"/>
			<ec:column property="ntn" title="TaxPayer NTN" escapeAutoFormat="true" />
			<ec:column property="cnic" title="TaxPayer CNIC" escapeAutoFormat="true" />
			<ec:column property="name" title="TaxPayer Name" escapeAutoFormat="true" />
			<ec:column property="city" title="TaxPayer City" escapeAutoFormat="true" />
			<ec:column property="address" title="TaxPayer Address" escapeAutoFormat="true" />
			<ec:column property="accountTypeName" title="TaxPayer Status" escapeAutoFormat="true" />
			<ec:column property="businessName" title="TaxPayer Business Name" escapeAutoFormat="true" />
			<ec:column property="totalCommission" title="Taxable Amount" escapeAutoFormat="true" calc="total" calcTitle="Total:"  cell="currency"  format="0.00" style="text-align: right"/>
			<ec:column property="totalWht" title="Tax Amount" escapeAutoFormat="true" calc="total" calcTitle="Total:"  cell="currency"  format="0.00" style="text-align: right"/>
