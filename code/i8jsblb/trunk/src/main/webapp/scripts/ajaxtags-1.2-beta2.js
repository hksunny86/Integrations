...
AjaxJspTag.UpdateField.prototype = Object.extend(new AjaxJspTag.Base(), {
  ...
  if( ($(this.options.action) != null && $(this.options.action) != undefined) &&  $(this.options.action).onclick!=null && $(this.options.action).onclick!=undefined)
   {
   	eval("$(this.options.action).on"+this.options.eventType+" = this.execute.bindAsEventListener(this)");
   //	alert('-->' +$(this.options.action).onclick);
  ...
  execute: function(e) {
    ...
    {// Imran Sarwar: Added this condition to handle the confirmation dialog
    ...
    var aj = new Ajax.Request(this.url, {
      ...
      // Imran Sarwar: this second parmater is added to identify the html component which is attached with this event
        if (obj.options.errorFunction != null) obj.options.errorFunction(request, obj.options.action);
      },
