 ...
 * this funciton will be available mostly in all the pages
 * through decorator.
 * If any decorator doesnot have this, add this function in that decorator main js.
 * pass '1000' for a 1 second animation
 * on calling jsp you must have Jquery added with following code
 * <script type="text/javascript">
 *		var jq=$.noConflict();
 *	</script>	
 */
function scrollToTop(scrollDuration) {
	jq('html, body').animate({scrollTop : 0},scrollDuration);
	return false;
};

function globalAjaxPostFunction()
{
	$('errorMsg').innerHTML = "";
...
function globalAjaxErrorFunction(request, obj) 
{
...
* 	Purpose:	This function checkes if user has changed values of form
*	Usage:		Call this function like below in you validate form method
*	
...
function isFormDataChanged()
{
	...
	for(var counter = 0; counter < inputArray.length; counter++){
		var element = inputArray[counter];
	...
	{	
		...
		for(var counter = 0; counter < selectArray.length; counter++){
			var element = selectArray[counter];
			if(element.multiple==false){
				...
				for(var x = 0; x < element.options.length; x++){
				    if (element.options[x].selected) 
				    	arSelected.push(element.options[x].value);
				}
			}
			if(formChanged)
				break;
	...
	{	
		...
		for(var counter = 0; counter < textareaArray.length; counter++){
			var element = textareaArray[counter];
			if(element.value!=element.defaultValue){
				formChanged = true;
				break;
			}
		...
		if(!formChanged)
		{
			alert("No changes have been made on screen.");
		}
	}
