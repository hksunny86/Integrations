...
//            }
    ...
    @RequestMapping(value = "api/coolingOff/releaseInProcessBalance", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
    public @ResponseBody
    ReleaseInProcessBalanceResponse releaseInProcessBalanceResponse(@Valid @RequestBody ReleaseInProcessBalanceRequest request) throws Exception {
        ...
        try {
            ...
            logger.info("Cooling Off Release InProcess Balance Request Received at Controller at time: " + start);
            String requestXML = JSONUtil.getJSON(request);
            //        requestXML = XMLUtil.maskPassword(requestXML);
            String datetime = "";
            SimpleDateFormat DateFor = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss a");
            ...
            StringBuilder stringText = new StringBuilder()
                    ...
                    .append(request.getReceiverMobileNo())
            ...
            if (request.getHashData().equalsIgnoreCase(sha256hex)) {
                if (CorporateHostRequestValidator.authenticate(request.getUserName(), request.getPassword(), request.getChannelId())) {
                    try {
                        CoolingOffRequestValidator.validateReleaseInProcessBalance(request);
                        response = coolingOffService.releaseInProcessBalanceResponse(request);
                    ...
                    logger.info("******* DEBUG LOGS FOR Cooling Off Release InProcess Balance Request *********");
                    logger.info("ResponseCode: " + response.getResponseCode());
                } else {
                    logger.info("******* DEBUG LOGS FOR Cooling Off Release InProcess Balance AUTHENTICATION *********");
                    response = new ReleaseInProcessBalanceResponse();
                    response.setResponseCode("420");
                    response.setResponseDescription("Request is not authenticated");
                    response.setRrn(request.getRrn());
                    response.setResponseDateTime(request.getDateTime());
        ...
        long end = System.currentTimeMillis() - start;
        String responseXML = JSONUtil.getJSON(response);
        logger.info("Cooling Off Release InProcess Balance Processed in : {} ms {}", end, Objects.requireNonNull(responseXML).replaceAll(System.getProperty("line.separator"), ""));
