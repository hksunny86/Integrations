...
//        logger.info("Start Processing Customer Name Update Request with {}", requestXML);
    ...
    UpdateCustomerInfoResponse updateCustomerInfoResponse(@RequestBody UpdateCustomerInfoRequest request) {
        ...
        if (request.getHashData().equalsIgnoreCase(sha256hex)) {
            if (HostRequestValidator.authenticate(request.getUserName(), request.getPassword(), request.getChannelId())) {
                try {
                    HostRequestValidator.validateUpdateCustomerInfo(request);
                    updateCustomerInfo = integrationService.updateCustomerInfoResponse(request);

                } catch (ValidationException ve) {
                    updateCustomerInfo.setResponseCode("420");
                    updateCustomerInfo.setResponseDescription(ve.getMessage());

                    logger.error("ERROR: Request Validation", ve);
                } catch (Exception e) {
                    updateCustomerInfo.setResponseCode("220");
                    updateCustomerInfo.setResponseDescription(e.getMessage());
                    logger.error("ERROR: General Processing ", e);
                }
                ...
                updateCustomerInfo.setResponseDateTime(request.getDateTime());
                logger.info("******* REQUEST IS NOT AUTHENTICATED *********");
            }
        } else {
            logger.info("******* DEBUG LOGS FOR Customer Info Update TRANSACTION *********");
            updateCustomerInfo = new UpdateCustomerInfoResponse();
            updateCustomerInfo.setResponseCode("111");
            updateCustomerInfo.setResponseDescription("Request is not recognized");
            logger.info("******* REQUEST IS NOT RECOGNIZED *********");
        }

        long end = System.currentTimeMillis() - start;
        logger.info("Customer Info Update Request  Processed in : {} ms {}", end, updateCustomerInfo);
    ...
    UpdateLimitResponse updateLimitResponse(@RequestBody UpdateLimitRequest request) {
        ...
        logger.info("Limit Update Request Recieve at Controller at time: " + start);
        String requestXML = XMLUtil.convertRequest(request);
        requestXML = XMLUtil.maskPassword(requestXML);
        String datetime = "";
        SimpleDateFormat DateFor = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss a");
        ...
        logger.info("Start Processing Update Limit Request with DateTime:" + datetime + " | URI: " + uri + " | IP: "
                + ip + " | GUID: " + guid + " {}", Objects.requireNonNull(requestXML).replaceAll(System.getProperty("line.separator"), " "));
        ...
        StringBuilder stringText = new StringBuilder()
                ...
                .append(request.getDailyReceivingLimit())
        ...
        if (request.getHashData().equalsIgnoreCase(sha256hex)) {
            if (HostRequestValidator.authenticate(request.getUserName(), request.getPassword(), request.getChannelId())) {
                try {
                    updateLimit = integrationService.updateLimit(request);

                } catch (ValidationException ve) {
                    updateLimit.setResponseCode("420");
                    updateLimit.setResponseDescription(ve.getMessage());

                    logger.error("ERROR: Request Validation", ve);
                } catch (Exception e) {
                    updateLimit.setResponseCode("220");
                    updateLimit.setResponseDescription(e.getMessage());
                    logger.error("ERROR: General Processing ", e);
                }

                logger.info("******* DEBUG LOGS FOR  Limit Update TRANSACTION *********");
                logger.info("ResponseCode: " + updateLimit.getResponseCode());
            } else {
                logger.info("******* DEBUG LOGS FOR Limit Update TRANSACTION AUTHENTICATION *********");
                updateLimit = new UpdateLimitResponse();
                updateLimit.setResponseCode("420");
                updateLimit.setResponseDescription("Request is not authenticated");
                updateLimit.setResponseDateTime(request.getDateTime());
                logger.info("******* REQUEST IS NOT AUTHENTICATED *********");
            }
        } else {
            logger.info("******* DEBUG LOGS FOR Limit Update TRANSACTION *********");
            updateLimit = new UpdateLimitResponse();
            updateLimit.setResponseCode("111");
            updateLimit.setResponseDescription("Request is not recognized");
            logger.info("******* REQUEST IS NOT RECOGNIZED *********");
        }

        long end = System.currentTimeMillis() - start;
        logger.info(" Update Limit Request  Processed in : {} ms {}", end, updateLimit);
...
//            logger.info("Start Processing Third Party Credit Request with {}", requestXML);
    ...
    @RequestMapping(value = "api/accountInfo", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
    public @ResponseBody
    AccountInfoResponse accountInfoResponse(@Valid @RequestBody AccountInfoRequest request) throws Exception {
        ...
        try {
            ...
            logger.info("Account Info Request Received at Controller at time: " + start);
            String requestXML = JSONUtil.getJSON(request);
            //        requestXML = XMLUtil.maskPassword(requestXML);
//            logger.info("Start Processing Third Party Credit Request with {}", requestXML);
            String datetime = "";
            SimpleDateFormat DateFor = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss a");
            datetime = DateFor.format(new Date());
            logger.info("Start Processing Account Info Request with DateTime:" + datetime + " | URI: " + uri + " | IP: "
                    + ip + " | GUID: " + guid + " {}", Objects.requireNonNull(requestXML).replaceAll(System.getProperty("line.separator"), " "));
            StringBuilder stringText = new StringBuilder()
                    .append(request.getUserName())
                    .append(request.getPassword())
            ...
            if (request.getHashData().equalsIgnoreCase(sha256hex)) {
                if (HostRequestValidator.authenticate(request.getUserName(), request.getPassword(), request.getChannelId())) {
                    try {
                        ...
                        response = integrationService.accountInfoResponse(request);
                    } catch (ValidationException ve) {
                        response.setResponseCode("420");
                        response.setResponseDescription(ve.getMessage());
                        logger.error("ERROR: Request Validation", ve);
    ...
    @RequestMapping(value = "api/glToGl", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
    public @ResponseBody
    GlToGlPaymentResponse glToGlPayment(@Valid @RequestBody GlToGlPaymentRequest request) throws Exception {
        ...
        try {
            ...
            logger.info("GL To GL Request Received at Controller at time: " + start);
            String requestXML = JSONUtil.getJSON(request);
            //        requestXML = XMLUtil.maskPassword(requestXML);
//            logger.info("Start Processing Third Party Credit Request with {}", requestXML);
            String datetime = "";
            SimpleDateFormat DateFor = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss a");
            datetime = DateFor.format(new Date());
            logger.info("Start Processing GL To GL Request with DateTime:" + datetime + " | URI: " + uri + " | IP: "
                    + ip + " | GUID: " + guid + " {}", Objects.requireNonNull(requestXML).replaceAll(System.getProperty("line.separator"), " "));
            StringBuilder stringText = new StringBuilder()
                    .append(request.getUserName())
                    .append(request.getPassword())
                    ...
                    .append(request.getChannelId())
                    .append(request.getTerminalId())
                    .append(request.getSenderGlAccountNo())
                    .append(request.getReceiverGlAccountNo())
                    .append(request.getAmount())
                    .append(request.getProductId())
                    .append(request.getReserved1())
                    .append(request.getReserved2())
            ...
            if (request.getHashData().equalsIgnoreCase(sha256hex)) {
                if (HostRequestValidator.authenticate(request.getUserName(), request.getPassword(), request.getChannelId())) {
                    try {
                        HostRequestValidator.validateGlToGl(request);
                        response = integrationService.glToGlPaymentResponse(request);
                    } catch (ValidationException ve) {
                        response.setResponseCode("420");
                        response.setResponseDescription(ve.getMessage());
                        logger.error("ERROR: Request Validation", ve);
                    ...
                    logger.info("******* DEBUG LOGS FOR GL To GL Request *********");
                    logger.info("ResponseCode: " + response.getResponseCode());
                } else {
                    logger.info("******* DEBUG LOGS FOR GL To GL Request AUTHENTICATION *********");
                    response = new GlToGlPaymentResponse();
                    response.setResponseCode("420");
                    response.setResponseDescription("Request is not authenticated");
                    logger.info("******* REQUEST IS NOT AUTHENTICATED *********");
                }
        ...
        long end = System.currentTimeMillis() - start;
        logger.info("GL To GL Request Processed in : {} ms {}", end, "");

        return response;
    }

    @RequestMapping(value = "api/checkLoan", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
    public @ResponseBody
    CheckLoanResponse checkLoan(@Valid @RequestBody CheckLoanRequest request) throws Exception {
        ...
        try {
            ...
            logger.info("Check Loan Status Request Received at Controller at time: " + start);
            String requestXML = JSONUtil.getJSON(request);
            //        requestXML = XMLUtil.maskPassword(requestXML);
//            logger.info("Start Processing Third Party Credit Request with {}", requestXML);
            String datetime = "";
            SimpleDateFormat DateFor = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss a");
            datetime = DateFor.format(new Date());
            logger.info("Start Processing Check Loan Status Request with DateTime:" + datetime + " | URI: " + uri + " | IP: "
                    + ip + " | GUID: " + guid + " {}", Objects.requireNonNull(requestXML).replaceAll(System.getProperty("line.separator"), " "));
            StringBuilder stringText = new StringBuilder()
                    .append(request.getUserName())
                    .append(request.getPassword())
            ...
            if (request.getHashData().equalsIgnoreCase(sha256hex)) {
                if (HostRequestValidator.authenticate(request.getUserName(), request.getPassword(), request.getChannelId())) {
                    try {
                        HostRequestValidator.validateCheckLoan(request);
                        response = integrationService.checkLoanResponse(request);
                    } catch (ValidationException ve) {
                        response.setResponseCode("420");
                        response.setResponseDescription(ve.getMessage());
                        logger.error("ERROR: Request Validation", ve);
                    ...
                    logger.info("******* DEBUG LOGS FOR Check Loan Status Request *********");
                    logger.info("ResponseCode: " + response.getResponseCode());
                } else {
                    logger.info("******* DEBUG LOGS FOR Check Loan Status Request AUTHENTICATION *********");
                    response = new CheckLoanResponse();
                    response.setResponseCode("420");
                    response.setResponseDescription("Request is not authenticated");
                    logger.info("******* REQUEST IS NOT AUTHENTICATED *********");
                }
        ...
        long end = System.currentTimeMillis() - start;
        logger.info("Check Loan Status Request Processed in : {} ms {}", end, "");

        return response;
    }

    @RequestMapping(value = "api/deviceUpdate", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
    public @ResponseBody
    DeviceUpdateResponse deviceUpdate(@Valid @RequestBody DeviceUpdateRequest request) throws Exception {
        ...
        try {
            ...
            logger.info("Device Update Request Received at Controller at time: " + start);
            String requestXML = JSONUtil.getJSON(request);
            //        requestXML = XMLUtil.maskPassword(requestXML);
//            logger.info("Start Processing Third Party Credit Request with {}", requestXML);
            String datetime = "";
            SimpleDateFormat DateFor = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss a");
            datetime = DateFor.format(new Date());
            logger.info("Start Processing Device Update Request with DateTime:" + datetime + " | URI: " + uri + " | IP: "
                    + ip + " | GUID: " + guid + " {}", Objects.requireNonNull(requestXML).replaceAll(System.getProperty("line.separator"), " "));
            StringBuilder stringText = new StringBuilder()
                    .append(request.getUserName())
                    .append(request.getPassword())
            ...
            if (request.getHashData().equalsIgnoreCase(sha256hex)) {
                if (HostRequestValidator.authenticate(request.getUserName(), request.getPassword(), request.getChannelId())) {
                    try {
                        HostRequestValidator.validateDeviceUpdate(request);
                        response = integrationService.deviceUpdateResponse(request);
                    } catch (ValidationException ve) {
                        response.setResponseCode("420");
                        response.setResponseDescription(ve.getMessage());
                        logger.error("ERROR: Request Validation", ve);
                    ...
                    logger.info("******* DEBUG LOGS FOR Device Update Request *********");
                    logger.info("ResponseCode: " + response.getResponseCode());
                } else {
                    logger.info("******* DEBUG LOGS FOR Device Update Request AUTHENTICATION *********");
        ...
        long end = System.currentTimeMillis() - start;
        logger.info("Device Update Request Processed in : {} ms {}", end, "");

        return response;
    }

    @RequestMapping(value = "api/dynamicQRPaymentInquiry", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
    public @ResponseBody
    DynamicQRPaymentInquiryResponse dynamicQRPaymentInquiry(@Valid @RequestBody DynamicQRPaymentInquiryRequest request) throws Exception {
        ...
        try {
            ...
            logger.info("Dynamic QR Payment Inquiry Request Received at Controller at time: " + start);
            String requestXML = JSONUtil.getJSON(request);
            //        requestXML = XMLUtil.maskPassword(requestXML);
//            logger.info("Start Processing Third Party Credit Request with {}", requestXML);
            String datetime = "";
            SimpleDateFormat DateFor = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss a");
            datetime = DateFor.format(new Date());
            logger.info("Start Processing Dynamic QR Payment Inquiry Request with DateTime:" + datetime + " | URI: " + uri + " | IP: "
                    + ip + " | GUID: " + guid + " {}", Objects.requireNonNull(requestXML).replaceAll(System.getProperty("line.separator"), " "));
            StringBuilder stringText = new StringBuilder()
                    .append(request.getUserName())
                    .append(request.getPassword())
            ...
            if (request.getHashData().equalsIgnoreCase(sha256hex)) {
                if (HostRequestValidator.authenticate(request.getUserName(), request.getPassword(), request.getChannelId())) {
                    try {
                        HostRequestValidator.validateDynamicQRPaymentInquiry(request);
                        response = integrationService.dynamicQRPaymentInquiryResponse(request);
                    } catch (ValidationException ve) {
                        response.setResponseCode("420");
                        response.setResponseDescription(ve.getMessage());
                        logger.error("ERROR: Request Validation", ve);
                    ...
                    logger.info("******* DEBUG LOGS FOR Dynamic QR Payment Inquiry Request *********");
                    logger.info("ResponseCode: " + response.getResponseCode());
                } else {
                    logger.info("******* DEBUG LOGS FOR Dynamic QR Payment Inquiry Request AUTHENTICATION *********");
                    response = new DynamicQRPaymentInquiryResponse();
                    response.setResponseCode("420");
                    response.setResponseDescription("Request is not authenticated");
                    logger.info("******* REQUEST IS NOT AUTHENTICATED *********");
                }
        ...
        long end = System.currentTimeMillis() - start;
        logger.info("Dynamic QR Payment Inquiry Request Processed in : {} ms {}", end, "");

        return response;
    }

    @RequestMapping(value = "api/debitRequest", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
    public @ResponseBody
    DebitJsonResponse debit(@Valid @RequestBody DebitJsonRequest request) throws Exception {
        ...
        try {
            ...
            logger.info(" Start Processing Debit Transaction Request Received at Controller at time: " + start);
            String requestXML = JSONUtil.getJSON(request);
            String datetime = "";
            SimpleDateFormat DateFor = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss a");
            datetime = DateFor.format(new Date());
...
//        if (request.getHashData().equalsIgnoreCase(sha256hex)) {
            if (HostRequestValidator.authenticate(request.getUserName(), request.getPassword(), request.getChannelId())) {
                try {
                    HostRequestValidator.debitJsonRequest(request);
                    response = integrationService.debitJsonResponse(request);
                ...
                response = new DebitJsonResponse();
                response.setResponseCode("420");
                response.setResponseDescription("Request is not authenticated");
                logger.info("******* REQUEST IS NOT AUTHENTICATED *********");
            }
...
//        }
        ...
        long end = System.currentTimeMillis() - start;
        String responseXML = JSONUtil.getJSON(response);
        logger.info("Debit Request Processed in : {} ms {}", end, Objects.requireNonNull(responseXML).replaceAll(System.getProperty("line.separator"), ""));
