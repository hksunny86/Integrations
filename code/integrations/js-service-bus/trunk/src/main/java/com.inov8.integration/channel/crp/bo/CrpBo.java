package com.inov8.integration.channel.crp.bo;

import com.inov8.integration.channel.crp.request.Request;
import com.inov8.integration.channel.crp.request.ScoreAndRatingRequest;
import com.inov8.integration.channel.crp.response.Response;
import com.inov8.integration.channel.crp.response.ScoreAnRatingResponse;
import com.inov8.integration.channel.crp.service.CrpService;
...
@Component
public class CrpBo implements I8SBChannelInterface {

    @Autowired
    CrpService crpService;
    private static Logger logger = LoggerFactory.getLogger(CrpBo.class.getSimpleName());
    ...
    public I8SBSwitchControllerResponseVO execute(I8SBSwitchControllerRequestVO i8SBSwitchControllerRequestVO) throws Exception {
        ...
        if (request.validateRequest()) {
            ...
            crpService.setI8SBSwitchControllerRequestVO(i8SBSwitchControllerRequestVO);
            if (requestType.equalsIgnoreCase(I8SBConstants.RequestType_GET_SCORE_AND_RATING)) {
                response = crpService.scoreAnRatingResponse((ScoreAndRatingRequest) request);
            }
    ...
    private Object[] initializeRequestAndResponseObjects(String requestType) {
        ...
        com.inov8.integration.channel.crp.request.Request request = null;
        com.inov8.integration.channel.crp.response.Response response = null;
        logger.info("Request type: " + requestType);
        if (requestType.equalsIgnoreCase(I8SBConstants.RequestType_GET_SCORE_AND_RATING)) {
            request = new ScoreAndRatingRequest();
            response = new ScoreAnRatingResponse();
        }
        else {
