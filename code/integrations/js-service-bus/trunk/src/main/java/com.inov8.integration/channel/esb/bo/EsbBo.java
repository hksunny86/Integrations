package com.inov8.integration.channel.esb.bo;
...
@Component
public class EsbBo implements I8SBChannelInterface {

    @Autowired
    EsbService esbService;
    private static Logger logger = LoggerFactory.getLogger(EsbBo.class.getSimpleName());
    ...
    public I8SBSwitchControllerResponseVO execute(I8SBSwitchControllerRequestVO i8SBSwitchControllerRequestVO) throws Exception {
        ...
        if (request.validateRequest()) {
            ...
            esbService.setI8SBSwitchControllerRequestVO(i8SBSwitchControllerRequestVO);
            if (requestType.equalsIgnoreCase(I8SBConstants.RequestType_ESB_BILL_INQUIRRY)) {
                response = esbService.esbBillInquiryResponse((EsbBillInquiryRequest) request);
            } else if (requestType.equalsIgnoreCase(I8SBConstants.RequestType_ESB_BILL_PAYMENT)) {
                response = esbService.esbBillPaymentResponse((EsbBillPaymentRequest) request);
            }
    ...
    private Object[] initializeRequestAndResponseObjects(String requestType) {
        ...
        if (requestType.equalsIgnoreCase(I8SBConstants.RequestType_ESB_BILL_INQUIRRY)) {
            request = new EsbBillInquiryRequest();
            response = new EsbBillInquiryResponse();

        } else if (requestType.equalsIgnoreCase(I8SBConstants.RequestType_ESB_BILL_PAYMENT)) {
            request = new EsbBillPaymentRequest();
            response = new EsbBillPaymentResponse();
