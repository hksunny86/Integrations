package com.inov8.integration.channel.euronet.bo;

import com.inov8.integration.channel.euronet.request.CardIssuanceRequest;
import com.inov8.integration.channel.euronet.request.Request;
import com.inov8.integration.channel.euronet.response.CardIssuanceResponse;
import com.inov8.integration.channel.euronet.response.Response;
import com.inov8.integration.channel.euronet.service.EuronetService;
...
@Component
public class EuroentBo implements I8SBChannelInterface {

    @Autowired
    EuronetService euronetService;
    private static Logger logger = LoggerFactory.getLogger(EuroentBo.class.getSimpleName());
    ...
    public I8SBSwitchControllerResponseVO execute(I8SBSwitchControllerRequestVO i8SBSwitchControllerRequestVO) throws Exception {
        ...
        if (request.validateRequest()) {
            ...
            euronetService.setI8SBSwitchControllerRequestVO(i8SBSwitchControllerRequestVO);
            if (requestType.equalsIgnoreCase(I8SBConstants.RequestType_EURONET_CRAD_ISSUANCE)) {
                response = euronetService.cardIssuance((CardIssuanceRequest) request);
            }
    ...
    private Object[] initializeRequestAndResponseObjects(String requestType) {
        ...
        if (requestType.equalsIgnoreCase(I8SBConstants.RequestType_EURONET_CRAD_ISSUANCE)) {
            request = new CardIssuanceRequest();
            response = new CardIssuanceResponse();
        } else {
