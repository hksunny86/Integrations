...
@Component
public class FcyBo implements I8SBChannelInterface {

    @Autowired
    FcyService fcyService;
    private static Logger logger = LoggerFactory.getLogger(FcyBo.class.getSimpleName());
    ...
    public I8SBSwitchControllerResponseVO execute(I8SBSwitchControllerRequestVO i8SBSwitchControllerRequestVO) throws Exception {
        ...
        if (request.validateRequest()) {
            ...
            fcyService.setI8SBSwitchControllerRequestVO(i8SBSwitchControllerRequestVO);
            if (requestType.equalsIgnoreCase(I8SBConstants.RequestType_FCY_CONVERSION)) {
                response = fcyService.fcyConversionResponse((FcyConversionRequest) request);
            }
    ...
    private Object[] initializeRequestAndResponseObjects(String requestType) {
        ...
        if (requestType.equalsIgnoreCase(I8SBConstants.RequestType_FCY_CONVERSION)) {
            request = new FcyConversionRequest();
            response = new FcyConversionResponse();
