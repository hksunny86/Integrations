package com.inov8.integration.channel.jazz.bo;

import com.inov8.integration.channel.jazz.request.Request;
import com.inov8.integration.channel.jazz.request.SendSmsRequest;
import com.inov8.integration.channel.jazz.response.Response;
import com.inov8.integration.channel.jazz.response.SendSmsResponse;
import com.inov8.integration.channel.jazz.service.JazzService;
...
@Component
public class JazzBo implements I8SBChannelInterface {

    @Autowired
    JazzService jazzService;
    private static Logger logger = LoggerFactory.getLogger(JazzBo.class.getSimpleName());
    ...
    public I8SBSwitchControllerResponseVO execute(I8SBSwitchControllerRequestVO i8SBSwitchControllerRequestVO) throws Exception {
        ...
        if (request.validateRequest()) {
            ...
            jazzService.setI8SBSwitchControllerRequestVO(i8SBSwitchControllerRequestVO);
            if (requestType.equalsIgnoreCase(I8SBConstants.RequestType_JAZZ_SEND_SMS)) {
                response = jazzService.sendSmsResponse((SendSmsRequest) request);
            }
