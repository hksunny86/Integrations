package com.inov8.integration.channel.merchant.bo;

import com.inov8.integration.channel.merchant.request.Request;
import com.inov8.integration.channel.merchant.request.StaticQrRequest;
import com.inov8.integration.channel.merchant.response.Response;
import com.inov8.integration.channel.merchant.response.StaticQrResponse;
import com.inov8.integration.channel.merchant.service.MerchantService;
...
@Component
public class MerchantBo implements I8SBChannelInterface {

    @Autowired
    MerchantService merchantService;
    private static Logger logger = LoggerFactory.getLogger(MerchantBo.class.getSimpleName());
    ...
    public I8SBSwitchControllerResponseVO execute(I8SBSwitchControllerRequestVO i8SBSwitchControllerRequestVO) throws Exception {
        ...
        if (request.validateRequest()) {
            ...
            merchantService.setI8SBSwitchControllerRequestVO(i8SBSwitchControllerRequestVO);
            if (requestType.equalsIgnoreCase(I8SBConstants.RequestType_MERCHANT_STATIC_QR)) {
                response = merchantService.staticQrResponse((StaticQrRequest) request);
            }
    ...
    private Object[] initializeRequestAndResponseObjects(String requestType) {
        ...
        if (requestType.equalsIgnoreCase(I8SBConstants.RequestType_MERCHANT_STATIC_QR)) {
            request = new StaticQrRequest();
            response = new StaticQrResponse();
