package com.inov8.integration.channel.merchantDiscountCamping.bo;


import com.inov8.integration.channel.merchantDiscountCamping.request.Request;
...
import com.inov8.integration.channel.merchantDiscountCamping.response.Response;
...
import com.inov8.integration.channel.merchantDiscountCamping.service.MerchantCampingService;
...
@Component
public class MerchantDiscountCampingBo implements I8SBChannelInterface {
    private static Logger logger = LoggerFactory.getLogger(MerchantDiscountCampingBo.class.getSimpleName());

    @Autowired
    MerchantCampingService merchantCampingService;
    ...
    public I8SBSwitchControllerResponseVO execute(I8SBSwitchControllerRequestVO i8SBSwitchControllerRequestVO) throws Exception {
        ...
        if (request.validateRequest()) {
            ...
            if (requestType.equalsIgnoreCase(I8SBConstants.RequestType_TransactionInquiry)) {
                response = merchantCampingService.transactionValidationResponse((TransactionValidationRequest) request);
            }
            if (requestType.equalsIgnoreCase(I8SBConstants.RequestType_TransactionStatus)) {
                response = merchantCampingService.transactionUpdateResponse((TransactionUpdateRequest) request);
    ...
    private Object[] initializeRequestAndResponseObjects(String requestType) {
        ...
        if (requestType.equalsIgnoreCase(I8SBConstants.RequestType_TransactionInquiry)) {
            request = new TransactionValidationRequest();
            response = new TransactionValidationResponse();
        } else if (requestType.equalsIgnoreCase(I8SBConstants.RequestType_TransactionStatus)) {
            request = new TransactionUpdateRequest();
            response = new TransactionUpdateResponse();
        } else {
