package com.inov8.integration.channel.pdm.bo;
...
import com.inov8.integration.channel.pdm.service.PdmService;
...
@Component
public class PdmBo implements I8SBChannelInterface {

    @Autowired
    PdmService pdmService;
    private static Logger logger = LoggerFactory.getLogger(PdmBo.class.getSimpleName());
    ...
    public I8SBSwitchControllerResponseVO execute(I8SBSwitchControllerRequestVO i8SBSwitchControllerRequestVO) throws Exception {
        ...
        if (request.validateRequest()) {
            ...
            pdmService.setI8SBSwitchControllerRequestVO(i8SBSwitchControllerRequestVO);
            if (requestType.equalsIgnoreCase(I8SBConstants.RequestType_PDM_Validate)) {
                response = pdmService.validatePdmResponse((ValidatePdmRequest) request);
            }
    ...
    private Object[] initializeRequestAndResponseObjects(String requestType) {
        ...
        if (requestType.equalsIgnoreCase(I8SBConstants.RequestType_PDM_Validate)) {
            request = new ValidatePdmRequest();
            response = new ValidatePdmResponse();
