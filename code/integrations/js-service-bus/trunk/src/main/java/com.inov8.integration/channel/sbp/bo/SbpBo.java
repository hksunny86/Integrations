package com.inov8.integration.channel.sbp.bo;

import com.inov8.integration.channel.sbp.request.Request;
import com.inov8.integration.channel.sbp.request.ReturnPaymentRequest;
import com.inov8.integration.channel.sbp.response.Response;
import com.inov8.integration.channel.sbp.response.ReturnPaymentResponse;
import com.inov8.integration.channel.sbp.service.SbpService;
...
@Component
public class SbpBo implements I8SBChannelInterface {

    @Autowired
    SbpService sbpService;
    private static Logger logger = LoggerFactory.getLogger(SbpBo.class.getSimpleName());
    ...
    public I8SBSwitchControllerResponseVO execute(I8SBSwitchControllerRequestVO i8SBSwitchControllerRequestVO) throws Exception {
        ...
        if (request.validateRequest()) {
            ...
            sbpService.setI8SBSwitchControllerRequestVO(i8SBSwitchControllerRequestVO);
            if (requestType.equalsIgnoreCase(I8SBConstants.RequestType_SBP_RETURN_PAYMENT)) {
                response = sbpService.returnPaymentResponse((ReturnPaymentRequest) request);
            }
    ...
    private Object[] initializeRequestAndResponseObjects(String requestType) {
        ...
        if (requestType.equalsIgnoreCase(I8SBConstants.RequestType_SBP_RETURN_PAYMENT)) {
            request = new ReturnPaymentRequest();
            response = new ReturnPaymentResponse();
        }
        else {
