package com.inov8.integration.channel.vision.bo;



import com.inov8.integration.channel.vision.request.Request;
import com.inov8.integration.channel.vision.response.Response;
...
import com.inov8.integration.channel.vision.service.VisionDebitCardService;
...
@Component
public class VisionDebitCardBO implements I8SBChannelInterface {
    private static final Logger logger = LoggerFactory.getLogger(VisionDebitCardBO.class.getSimpleName());
    @Autowired
    VisionDebitCardService visionDebitCardService;
    @Override
    public I8SBSwitchControllerResponseVO execute(I8SBSwitchControllerRequestVO i8SBSwitchControllerRequestVO) throws Exception {
        ...
        if (request.validateRequest()) {
            ...
            visionDebitCardService.setI8SBSwitchControllerRequestVO(i8SBSwitchControllerRequestVO);

            if (requestType.equalsIgnoreCase(I8SBConstants.RequestType_VISION_DEBIT_CARD)) {

                response = visionDebitCardService.visionDebitCardResponse((VisionDebitCardRequest) request);
    ...
    private Object[] initializeRequestAndResponseObjects(String requestType) throws I8SBValidationException {
        ...
        if (requestType.equalsIgnoreCase(I8SBConstants.RequestType_VISION_DEBIT_CARD)) {
            request = new VisionDebitCardRequest();
            response = new VisionDebitCardResponse();
        } else {
