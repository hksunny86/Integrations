...
/* 237 */     int max = widths[0]; int i;
/* 238 */     for (i = 0; i < widths.length; i++) {
/* 239 */       int width = widths[i];
/* 240 */       max = Math.max(width, max);
/*     */     } 
/*     */     
/* 243 */     for (i = 0; i < widths.length; i++) {
/* 244 */       widths[i] = max;
/*     */     }
/*     */   }
/*     */   
...
/* 299 */     return new Dimension(insets.left + insets.right + countGap(hInfo, 0, hInfo.getCellCount()) + this.myMargin.left + this.myMargin.right, insets.top + insets.bottom + countGap(vInfo, 0, vInfo.getCellCount()) + this.myMargin.top + this.myMargin.bottom);
/*     */   }
/*     */ 
/*     */ 
/*     */   
...
/* 328 */     int counter = 0;
/* 329 */     int cellIndex = startCell + cellCount - 2;
/* 330 */     for (; cellIndex >= startCell; 
/* 331 */       cellIndex--) {
...
/*     */     
/* 347 */     int indexOfNextNotEmpty = -1; int i;
/* 348 */     for (i = cellIndex + 1; i < info.getCellCount(); i++) {
...
/*     */       } 
/*     */     } 
/* 355 */     for (i = 0; i < info.getComponentCount(); i++) {
/* 356 */       Component component = info.getComponent(i);
/* 357 */       if (!(component instanceof Spacer)) {
/*     */ 
/*     */ 
/*     */         
/* 361 */         if (info.componentBelongsCell(i, cellIndex) && DimensionInfo.findAlignedChild(component, info.getConstraints(i)) != null)
/*     */         {
/* 363 */           return true;
/*     */         }
/*     */         
...
/* 436 */       for (int j = 0; j < heights.length; j++) {
/* 437 */         this.myYs[j] = y;
/* 438 */         this.myHeights[j] = heights[j];
/* 439 */         y += heights[j];
/* 440 */         if (shouldAddGapAfterCell(verticalInfo, j)) {
...
/* 465 */       for (int j = 0; j < widths.length; j++) {
/* 466 */         this.myXs[j] = x;
/* 467 */         this.myWidths[j] = widths[j];
/* 468 */         x += widths[j];
/* 469 */         if (shouldAddGapAfterCell(horizontalInfo, j)) {
...
/*     */         } 
/* 556 */         this.myHeights[this.myRowStretches.length - 1] = this.myHeights[this.myRowStretches.length - 1] - insets.bottom + this.myMargin.bottom;
/* 557 */         skipLayout |= 0x1;
/*     */       } 
...
/*     */         } 
/* 567 */         this.myWidths[this.myColumnStretches.length - 1] = this.myWidths[this.myColumnStretches.length - 1] - insets.right + this.myMargin.right;
/* 568 */         skipLayout |= 0x2;
/*     */       } 
/*     */     } 
...
/* 634 */     int[] widths = new int[info.getCellCount()]; int i;
/* 635 */     for (i = 0; i < widths.length; i++) {
/* 636 */       widths[i] = this.myMinCellSize;
/*     */     }
/*     */ 
/*     */     
/* 640 */     for (i = info.getComponentCount() - 1; i >= 0; i--) {
...
/*     */     
/* 660 */     for (int j = info.getComponentCount() - 1; j >= 0; j--) {
/* 661 */       int size = min ? getMin2(info, j) : Math.max(info.getMinimumWidth(j), info.getPreferredWidth(j));
/*     */       
/* 663 */       int span = info.getSpan(j);
/* 664 */       int cell = info.getCell(j);
/*     */       
...
/*     */       
/* 671 */       int curSize = 0;
/* 672 */       for (int k = 0; k < span; k++) {
/* 673 */         curSize += widths[k + cell];
/* 674 */         toProcess[k + cell] = true;
/*     */       } 
/*     */       
/* 677 */       if (curSize < size) {
/*     */ 
...
/* 758 */     for (int j = cell; j < cell + span; j++) {
/* 759 */       allowedCells[j] = true;
/*     */     }
/*     */     
...
/* 778 */     for (int j = 0; j < info.getCellCount(); j++) {
/* 779 */       if (higherPriorityCells[j]) {
/*     */ 
/*     */ 
/*     */         
/* 783 */         int addon = toDistributeFrozen * info.getStretch(j) / stretches;
/* 784 */         widths[j] = widths[j] + addon;
/*     */         
...
/* 791 */       for (int k = 0; k < info.getCellCount(); k++) {
/* 792 */         if (higherPriorityCells[k]) {
/*     */ 
/*     */ 
/*     */           
/* 796 */           widths[k] = widths[k] + 1;
/* 797 */           toDistribute--;
/*     */           
...
/* 824 */       for (int i = 0; i < allowedCells.length; i++) {
/* 825 */         if (allowedCells[i])
/*     */         {
/*     */           
/* 828 */           if (!isCellEmpty(info, i) && prefs[i] > widths[i]) {
/* 829 */             higherPriorityCells[i] = true;
/* 830 */             foundCells++;
/*     */           } 
/*     */         }
/*     */       } 
...
/*     */     
/*     */     int cell;
/* 840 */     for (cell = 0; cell < allowedCells.length; cell++) {
/* 841 */       if (allowedCells[cell])
/*     */       {
/*     */         
...
/*     */ 
/*     */     
/* 855 */     for (cell = 0; cell < allowedCells.length; cell++) {
/* 856 */       if (allowedCells[cell])
/*     */       {
/*     */         
...
/*     */ 
/*     */     
/* 870 */     for (cell = 0; cell < allowedCells.length; cell++) {
/* 871 */       if (allowedCells[cell])
/*     */       {
/*     */         
...
/*     */ 
/*     */     
/* 885 */     for (cell = 0; cell < allowedCells.length; cell++) {
/* 886 */       if (allowedCells[cell])
/*     */       {
/*     */         
...
/* Location:              D:\jsbank\Encryption_Utility\EncryptionUtility.jar!\com\intelli\\uiDesigner\core\GridLayoutManager.class
